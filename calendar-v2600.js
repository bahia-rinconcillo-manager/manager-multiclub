/* V24.5.8 — Módulo de calendario estable por equipo */
(function(){
  "use strict";
  ...
  function competitionLogo(name,className=""){
    ...
    if(!source)return `<span class="v258-competition-fallback ${className}">?</span>`;
  ...
  function renderCalendar(team){
    ...
    content.innerHTML=`<section class="v258-calendar-shell">
      ...
      <div class="v258-calendar-section-title"><div><small>CALENDARIO DEL EQUIPO</small><h4>${matches.length?`${matches.length} partido${matches.length===1?"":"s"}`:"Sin partidos"}</h4></div><span>Vista en dos columnas</span></div>
      <div class="v258-match-grid">${matches.length?matches.map(renderCard).join(""):`<div class="v258-empty"><img src="assets/escudo-oficial.png" alt="Escudo"><strong>No hay partidos registrados</strong><p>Pulsa “Nuevo partido” para añadir el primer encuentro de ${safeText(team.name)}.</p><button type="button" onclick="calendarV258.newMatch()">+ Crear partido</button></div>`}</div>
    </section>`;
  }
