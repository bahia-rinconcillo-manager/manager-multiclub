// V1.0.27E · Ficha de jugador consultable en histórico y bloqueo de edición
...
const MULTICLUB_BUILD_VERSION="V1.0.27E";
...
function setHistoricalPlayerDialogMode(enabled){
  if(!playerFormEl||!playerDialogEl)return;
  const historical=Boolean(enabled);
  playerDialogEl.classList.toggle("historical-player-record",historical);

  let notice=playerDialogEl.querySelector("#historicalPlayerRecordNotice");
  if(historical&&!notice){
    notice=document.createElement("div");
    notice.id="historicalPlayerRecordNotice";
    notice.className="historical-player-record-notice";
    notice.innerHTML=`<strong>Temporada ${esc(currentSeasonLabel())} · SOLO LECTURA</strong><span>Puedes consultar la ficha completa. Ningún dato de esta temporada puede modificarse.</span>`;
    playerFormEl.querySelector(".modal-head")?.insertAdjacentElement("afterend",notice);
  }else if(historical&&notice){
    notice.innerHTML=`<strong>Temporada ${esc(currentSeasonLabel())} · SOLO LECTURA</strong><span>Puedes consultar la ficha completa. Ningún dato de esta temporada puede modificarse.</span>`;
  }
  if(notice)notice.classList.toggle("hidden",!historical);

  playerFormEl.querySelectorAll("input,select,textarea").forEach(field=>{
    if(field.type==="hidden")return;
    if(historical){
      if(!field.hasAttribute("data-history-player-state")){
        field.dataset.historyPlayerState=JSON.stringify({disabled:!!field.disabled,readOnly:!!field.readOnly});
      }
      if(field.tagName==="SELECT"||field.type==="file")field.disabled=true;
      else field.readOnly=true;
      field.dataset.historyPlayerReadonly="true";
    }else if(field.hasAttribute("data-history-player-state")){
      try{
        const state=JSON.parse(field.dataset.historyPlayerState||"{}");
        field.disabled=!!state.disabled;
        field.readOnly=!!state.readOnly;
      }catch(_){field.disabled=false;field.readOnly=false}
      delete field.dataset.historyPlayerState;
      delete field.dataset.historyPlayerReadonly;
    }
  });

  const submit=playerFormEl.querySelector('button[type="submit"]');
  if(submit){
    submit.disabled=historical;
    submit.dataset.historyPlayerSubmit=historical?"true":"false";
  }
  if(historical){
    deletePlayerFromForm?.classList.add("hidden");
    deletePlayerFromForm?.removeAttribute("data-player-id");
  }
  const closeText=playerFormEl.querySelector('.modal-actions-right [data-close="playerDialog"]');
  if(closeText)closeText.textContent=historical?"Cerrar":"Cancelar";
}
...
function openPlayerRecord(id){
  ...
  // Restaurar primero el formulario por si venimos de una consulta histórica.
  setHistoricalPlayerDialogMode(false);
  playerFormEl.reset();
  ...
  setHistoricalPlayerDialogMode(isHistoricalSeason());
}
window.openPlayerRecord=openPlayerRecord;

// Apertura delegada: funciona también cuando la temporada histórica ha
// desactivado el resto de botones de escritura.
document.addEventListener("click",event=>{
