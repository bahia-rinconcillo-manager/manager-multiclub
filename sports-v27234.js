/* CD San Bernabé Manager V27.2.2 · Seguimiento deportivo multitemporada */
(function(){
  "use strict";
  ...
  function initialiseFilters(){
    ...
    if($("sportsMatchMonth")&&!$("sportsMatchMonth").value)$("sportsMatchMonth").value=currentMonth();
  ...
  async function load(){
    ...
    try{
      const {data,error}=await sb.rpc("sports_snapshot",{p_token:token()});
      ...
      return true;
    }catch(error){rpcError(error,"Seguimiento deportivo");renderActiveTeamPanel(error);return false}
  }
  ...
  async function saveTraining(event){
    ...
    try{
      ...
      const {data,error}=await sb.rpc("sports_save_training",{p_token:token(),p_payload:payload,p_attendance:collectTrainingAttendance()});
      if(error)throw error;
  ...
  async function deleteTraining(id){
    ...
    try{
      const {data,error}=await sb.rpc("sports_delete_training",{p_token:token(),p_id:id});
      if(error)throw error;
      const result=recordResult(data);
      if(result?.error_message)throw new Error(result.error_message);
  ...
  function filteredMatches(){
    ...
    return matches.filter(item=>(!teamId||String(item.team_id)===String(teamId))&&(!month||String(item.match_date||"").startsWith(month))&&(!search||lower(`${item.opponent} ${item.competition} ${item.venue}`).includes(search)));
  }
  ...
  function matchSummary(id){
    ...
    return {
      ...
      minutes:list.reduce((sum,x)=>sum+number(x.minutes),0),
  ...
  function matchRosterHtml(teamId,matchId=""){
    ...
    return `<div class="sports-roster-toolbar"><button type="button" data-mark-match="Convocado">Todos convocados</button><button type="button" data-mark-match="No convocado">Limpiar convocatoria</button></div>
      <div class="sports-match-roster">${list.map(player=>{
        const row=existing.get(String(player.id))||{participation_status:"No convocado",minutes:0,yellow_cards:0,red_cards:0,notes:""};
        const minuteDisabled=["No convocado","Ausente","Lesionado","Convocado"].includes(row.participation_status);
  ...
  function openMatch(id="",presetTeamId=""){
    if(historical())return notify("La temporada histórica es de solo lectura");
    const dialog=$("sportsMatchDialog");
    const form=$("sportsMatchForm");
    if(!dialog||!form)return;
    const item=matches.find(x=>String(x.id)===String(id));
  ...
  async function saveMatch(event){
    ...
    try{
      ...
      const {data,error}=await sb.rpc("sports_save_match",{p_token:token(),p_payload:payload,p_player_stats:collectMatchStats()});
      if(error)throw error;
      const result=recordResult(data);
      if(result?.error_message)throw new Error(result.error_message);
  ...
  async function deleteMatch(id){
    ...
    try{
      const {data,error}=await sb.rpc("sports_delete_match",{p_token:token(),p_id:id});
      if(error)throw error;
      const result=recordResult(data);
      if(result?.error_message)throw new Error(result.error_message);
  ...
  function teamMatches(teamId){
    const month=teamPanelFilters.matchesMonth||currentMonth();
    const search=lower(teamPanelFilters.matchSearch);
    return matches.filter(item=>String(item.team_id)===String(teamId)&&(!month||String(item.match_date||"").startsWith(month))&&(!search||lower(`${item.opponent} ${item.competition} ${item.venue}`).includes(search)))
  ...
  function renderTeamMatches(teamId,container){
    ...
    container.innerHTML=`${teamPanelHeader("Partidos, minutos y tarjetas","Registra convocatoria, titularidad, minutos jugados y sanciones de la plantilla.",`<button type="button" class="primary" data-sports-action="true" onclick="sportsV2703.newMatchForTeam('${safe(teamId)}')">+ Nuevo partido</button>`)}
      <div class="team-sports-toolbar"><label>Mes<input type="month" value="${safe(teamPanelFilters.matchesMonth||currentMonth())}" onchange="sportsV2703.setTeamFilter('matchesMonth',this.value)"></label><label class="grow">Buscar<input value="${safe(teamPanelFilters.matchSearch)}" placeholder="Rival, competición o campo" oninput="sportsV2703.setTeamFilter('matchSearch',this.value)"></label></div>
      <div class="sports-kpis team-sports-kpis"><article><small>Partidos</small><strong>${list.length}</strong><span>Encuentros del mes</span></article><article><small>Minutos</small><strong>${summaries.reduce((s,x)=>s+x.minutes,0)}</strong><span>Total de la plantilla</span></article><article><small>Amarillas</small><strong>${summaries.reduce((s,x)=>s+x.yellow,0)}</strong><span>Tarjetas registradas</span></article><article><small>Rojas</small><strong>${summaries.reduce((s,x)=>s+x.red,0)}</strong><span>Tarjetas registradas</span></article></div>
  ...
  function bind(){
    ...
    $("sportsMatchRoster")?.addEventListener("click",event=>{
      ...
      document.querySelectorAll("#sportsMatchRoster .sports-match-player").forEach(row=>{
        ...
        const input=row.querySelector("[data-match-minutes]");input.value="0";input.disabled=true;
      });
    });
    $("sportsMatchRoster")?.addEventListener("change",event=>{
      ...
      const disabled=["No convocado","Ausente","Lesionado","Convocado"].includes(select.value);
  ...
  window.sportsV2703=window.sportsV2702=window.sportsV2701=window.sportsV2700={
    ...
    newMatch:()=>openMatch(),newMatchForTeam:teamId=>openMatch("",teamId),editMatch:openMatch,deleteMatch,
