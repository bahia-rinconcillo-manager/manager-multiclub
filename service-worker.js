const APP_CACHE = "manager-multiclub-app-v1-0-40";
...
self.addEventListener("install",event=>{
  event.waitUntil(caches.open(APP_CACHE).then(async cache=>{
    ...
    if(failed)console.warn(`Manager Multiclub V1.0.40: ${failed} recursos se cargarán bajo demanda.`);
  }));
  self.skipWaiting();
});
