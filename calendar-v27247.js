...
(function(){
  ...
  function renderCard(match){
    ...
    return `<article class="v258-match-card ${state.className}">
      ...
      <div class="v258-scoreboard">
        <div class="v258-side">${crest(homeName,normalize(homeName)===normalize(match.team),match)}<strong>${safeText(homeName)}</strong><small>LOCAL</small></div>
        <div class="v258-score">${score}</div>
        <div class="v258-side">${crest(awayName,normalize(awayName)===normalize(match.team),match)}<strong>${safeText(awayName)}</strong><small>VISITANTE</small></div>
      </div>
  ...
  function renderListItem(match){
    ...
    return `<article class="v243-list-row ${state.className}">
      ...
      <div class="v243-list-versus">
        <div class="v243-list-team">${crest(homeName,normalize(homeName)===normalize(match.team),match)}<strong>${safeText(homeName)}</strong><small>LOCAL</small></div>
        <div class="v243-list-score">${safeText(score)}</div>
        <div class="v243-list-team">${crest(awayName,normalize(awayName)===normalize(match.team),match)}<strong>${safeText(awayName)}</strong><small>VISITANTE</small></div>
      </div>
  ...
  function openPdf(teamName){
    openChild("matchesPdfDialog",()=>{
      ...
      if(file)file.value="";
      if(status)status.textContent="Selecciona el calendario PDF de la RFAF. Al elegirlo se analizará automáticamente; también puedes pulsar «Analizar PDF».";
      if(team)team.value=teamName||"";
      if(competition&&(!competition.value||competition.value==="Liga AAFB"))competition.value="Liga RFAF";
      if(typeof window.ensurePdfImportBindingsV27245==="function")window.ensurePdfImportBindingsV27245();
      renderPdfPreview();
    });
  }
