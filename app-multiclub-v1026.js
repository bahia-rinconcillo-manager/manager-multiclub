...
async function applyTechnicalClubContext(profile){
  ...
  const {data,error}=await sb.from("clubes").select("id,nombre,nombre_corto,slug,escudo_url,activo").eq("id",clubId).eq("activo",true).maybeSingle();
  if(error)throw new Error("No se pudo cargar el club del acceso técnico: "+(error.message||"error desconocido"));
  if(!data)throw new Error("El club de este acceso técnico no está disponible.");
  currentMulticlubClub={...data,rol:"technical"};
