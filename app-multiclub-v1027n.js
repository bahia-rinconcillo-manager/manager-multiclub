// V1.0.27N · Temporada activa robusta + guardado de equipos corregido
// V1.0.27M · Nuevo equipo/evento + PDF semanal
// V1.0.27K · Reparación apertura Nuevo jugador
// V1.0.27J · Nuevo diseño de fichas de jugadores y técnicos
...
const MULTICLUB_BUILD_VERSION="V1.0.27N";
...
const playerDialogTitleEl=getEl("playerDialogTitle");
const playerDialogSubtitleEl=getEl("playerDialogSubtitle");
...
const staffDialogSubtitle=getEl("staffDialogSubtitle");
const staffForm=getEl("staffForm");
const staffFormPhotoPreview=getEl("staffFormPhotoPreview");
const currentStaffPhotoFile=getEl("currentStaffPhotoFile");
...
function setProfileImagePreview(img,src){
  if(!img)return;
  img.src=src||placeholderPhoto;
  img.onerror=()=>{img.onerror=null;img.src=placeholderPhoto};
}
function previewSelectedImage(input,img){
  const file=input?.files?.[0];
  if(!file)return;
  const url=URL.createObjectURL(file);
  setProfileImagePreview(img,url);
  img.onload=()=>URL.revokeObjectURL(url);
}
function renderPlayerProfilePaymentSummary(playerId){
  const box=document.getElementById("playerFormPaymentSummary");
  if(!box)return;
  if(!playerId){
    box.innerHTML='<div class="profile-payment-empty">Los conceptos de cobro aparecerán después de guardar el jugador.</div>';
    return;
  }
  const rows=(payments||[]).filter(row=>String(row?.player_id)===String(playerId));
  const conceptLabel=key=>{
    const concept=(paymentConcepts||[]).find(row=>String(row?.concept_key)===String(key));
    return concept?.name||key||"Concepto";
  };
  ...
  box.innerHTML=(rows.length?rows.map(row=>`<div class="profile-payment-row"><span>${esc(conceptLabel(row.concept))}</span><strong>${esc(row.status||"Pendiente")}</strong><b>${euro(row.amount||0)}</b></div>`).join(""):'<div class="profile-payment-empty">No hay cobros registrados para este jugador.</div>')+`<div class="profile-payment-total"><span>Pagado <strong>${euro(paid)}</strong></span><span>Pendiente <strong>${euro(due)}</strong></span></div>`;
}
playerFormEl?.elements?.photo_file?.addEventListener("change",event=>previewSelectedImage(event.currentTarget,playerFormPhotoPreviewEl));
staffForm?.elements?.photo_file?.addEventListener("change",event=>previewSelectedImage(event.currentTarget,staffFormPhotoPreview));

function setStaffDialogReadOnly(readOnly){
  if(!staffDialog||!staffForm)return;
  staffDialog.classList.toggle("profile-readonly",Boolean(readOnly));
  staffForm.querySelectorAll("input,select,textarea").forEach(field=>{
    if(field.type==="hidden")return;
    if(readOnly){
      if(field.disabled)field.dataset.profileWasDisabled="true";
      field.disabled=true;
    }else{
      if(field.dataset.profileWasDisabled==="true"){
        field.disabled=true;
        delete field.dataset.profileWasDisabled;
      }else field.disabled=false;
    }
  });
  const backpack=staffForm.querySelector('input[value="Incluida por defecto"]');
  if(backpack)backpack.disabled=true;
  const closeButton=staffForm.querySelector('.profile-sheet-actions [data-close="staffDialog"]');
  if(closeButton)closeButton.textContent=readOnly?"Cerrar":"Cancelar";
}
...
async function loadClubSeasons(){
  ...
  try{
    ...
    console.warn("Gestión de temporadas no disponible en la consulta completa",error);
    seasonSystemReady=false;
    // V1.0.27N: no continuar silenciosamente con season_id=null. Intentamos
    // recuperar al menos id/nombre/estado de la temporada activa del club.
    try{
      activeClubSeason=null;selectedClubSeason=null;clubSeasons=[];
      await ensureActiveClubSeasonV1027N();
      return true;
    }catch(recoveryError){
      console.error("No se pudo recuperar la temporada activa",recoveryError);
      seasonFallback();renderSeasonSelector();
      if(syncState){syncState.textContent="● Error de temporada";syncState.title=recoveryError?.message||String(recoveryError)}
      return false;
    }
  }
}
async function ensureActiveClubSeasonV1027N(){
  if(activeClubSeason?.id)return activeClubSeason;
  if(typeof isTechnicalReadOnly==="function"&&isTechnicalReadOnly()){
    const technical=window.__technicalAccess||currentAccess||{};
    if(!technical.season_id)throw new Error("La sesión técnica no contiene una temporada activa válida");
    const name=technical.season_name||"2026/27";
    const startYear=Number(technical.season_start_year)||Number(String(name).slice(0,4))||2026;
    const season={id:technical.season_id,name,start_year:startYear,end_year:Number(technical.season_end_year)||startYear+1,status:"active",created_at:null,closed_at:null};
    clubSeasons=[season];activeClubSeason=season;selectedClubSeason=season;seasonSystemReady=true;
    setSeasonState(season);renderSeasonSelector();
    return season;
  }
  ...
  let fullError=null;
  try{
    const {data,error}=await sb.from("club_seasons")
      .select("id,name,start_year,end_year,status,created_at,closed_at")
      .eq("status","active")
      .limit(1);
    if(error)throw error;
    const row=Array.isArray(data)?data[0]:null;
    if(row?.id){
      const existing=(clubSeasons||[]).filter(x=>String(x?.id)!==String(row.id));
      clubSeasons=[row,...existing];activeClubSeason=row;
      if(!selectedClubSeason?.id)selectedClubSeason=row;
      seasonSystemReady=true;setSeasonState(selectedClubSeason||row);renderSeasonSelector();
      return row;
    }
  }catch(error){fullError=error;console.warn("V1.0.27N · recuperación completa de temporada",error)}

  try{
    const {data,error}=await sb.from("club_seasons").select("id,name,status").eq("status","active").limit(1);
    if(error)throw error;
    const row=Array.isArray(data)?data[0]:null;
    if(!row?.id)throw new Error("No existe una temporada activa para este club");
    const match=String(row.name||"").match(/^(\d{4})\/(\d{2})$/);
    const startYear=match?Number(match[1]):2026;
    const season={...row,start_year:startYear,end_year:startYear+1,created_at:null,closed_at:null};
    const existing=(clubSeasons||[]).filter(x=>String(x?.id)!==String(season.id));
    clubSeasons=[season,...existing];activeClubSeason=season;
    if(!selectedClubSeason?.id)selectedClubSeason=season;
    seasonSystemReady=true;setSeasonState(selectedClubSeason||season);renderSeasonSelector();
    return season;
  }catch(error){
    console.error("V1.0.27N · no se pudo recuperar la temporada activa",error);
    const detail=error?.message||fullError?.message||"Error desconocido";
    throw new Error(`No se pudo identificar la temporada activa del club. ${detail}`);
  }
}
...
async function loadTrashAndActivity(){
  if(!activeClubSeason?.id&&!isTechnicalReadOnly())await ensureActiveClubSeasonV1027N();
...
async function loadAll(){
  ...
  if(!isTechnicalReadOnly()&&!isHistoricalSeason()&&!activeClubSeason?.id){
    try{await ensureActiveClubSeasonV1027N()}
    catch(error){
      console.error("V1.0.27N · carga bloqueada por temporada inválida",error);
      syncState.textContent="● Error de temporada";syncState.title=error?.message||String(error);
      showOperationError("No se pueden cargar los datos porque no se ha podido identificar la temporada activa.",error);
      return;
    }
  }
...
function renderStaff(){
  const orderedStaff=sortStaffAlpha(staff);
  staffBody.innerHTML=orderedStaff.length?orderedStaff.map(s=>`<tr><td><strong>${esc(s.name)}</strong></td><td>${esc(s.role||"-")}</td><td>${esc(s.team_name||"-")}</td><td>${esc(s.phone||"-")}</td><td>${esc(s.email||"-")}</td><td><button class="mini" type="button" data-season-readonly-allow="true" onclick="editStaff('${s.id}')">Abrir ficha</button>${isHistoricalSeason()?"":` <button class="mini danger-mini" onclick="deleteStaff('${s.id}')">Eliminar</button>`}</td></tr>`).join(""):'<tr><td colspan="6">No hay miembros registrados.</td></tr>';
}
...
/* V1.0.27M · Restauración robusta de Equipos y Calendario */
function fillCoachOptionsV1027M(selected=""){
  if(!teamForm?.elements?.coach_id)return;
  teamForm.elements.coach_id.innerHTML='<option value="">Sin asignar</option>'+sortStaffAlpha((staff||[]).filter(s=>s.role==="Entrenador")).map(s=>`<option value="${esc(s.id)}">${esc(s.name)}</option>`).join("");
  teamForm.elements.coach_id.value=selected||"";
}
function openNewTeamForm(){
  if(isHistoricalSeason())return toast("El histórico es solo de consulta");
  if(!teamDialog||!teamForm){console.error("No se encuentra el formulario de equipos");return toast("No se encuentra el formulario de equipos")}
  try{teamForm.reset()}catch(error){console.warn("No se pudo reiniciar el formulario de equipo",error)}
  try{if(teamForm.elements.id)teamForm.elements.id.value=""}catch(_){ }
  try{deleteTeamFromForm?.classList.add("hidden");deleteTeamFromForm?.removeAttribute("data-team-id")}catch(_){ }
  try{fillCoachOptionsV1027M()}catch(error){console.warn("No se pudieron cargar entrenadores",error)}
  try{
    if(teamDialog.open)teamDialog.close();
    if(typeof teamDialog.showModal==="function")teamDialog.showModal();
    else teamDialog.setAttribute("open","");
  }catch(error){console.error("No se pudo abrir Nuevo equipo",error);teamDialog.setAttribute("open","")}
}
window.openNewTeamForm=openNewTeamForm;
addTeam?.addEventListener("click",openNewTeamForm);
window.editTeam=id=>{
  const team=(teams||[]).find(x=>String(x.id)===String(id));
  if(!team)return toast("Equipo no encontrado");
  try{teamForm.reset()}catch(_){ }
  try{deleteTeamFromForm?.classList.remove("hidden");deleteTeamFromForm?.setAttribute("data-team-id",id)}catch(_){ }
  try{fillCoachOptionsV1027M(team.coach_id)}catch(_){ }
  ["id","name","age_category","category","delegate_name","training_schedule","field","notes"].forEach(name=>{if(teamForm?.elements?.[name])teamForm.elements[name].value=team[name]||""});
  try{if(typeof teamDialog.showModal==="function")teamDialog.showModal();else teamDialog.setAttribute("open","")}catch(_){teamDialog.setAttribute("open","")}
};
if(teamForm)teamForm.onsubmit=async event=>{
  event.preventDefault();
  if(isHistoricalSeason())return toast("El histórico es solo de consulta");
  const raw=Object.fromEntries(new FormData(teamForm));
  const id=raw.id||null;
  try{
    const season=await ensureActiveClubSeasonV1027N();
    if(!season?.id)throw new Error("No hay una temporada activa válida. El equipo no se ha guardado.");
    const data={name:(raw.name||"").trim(),age_category:(raw.age_category||"").trim()||null,category:(raw.category||"").trim()||null,coach_id:raw.coach_id||null,delegate_name:(raw.delegate_name||"").trim()||null,training_schedule:(raw.training_schedule||"").trim()||null,field:(raw.field||"").trim()||null,notes:(raw.notes||"").trim()||null,season_id:season.id};
    ...
    await logActivity(id?"actualizar":"crear","equipo",id||null,`${id?"Equipo actualizado":"Equipo creado"}: ${data.name}`);
    teamDialog.close();toast("Equipo guardado correctamente");await loadAll();
  }catch(error){console.error(error);toast("Equipo: "+(error.message||"No se pudo guardar"))}
};

function openNewEventForm(){
  if(isHistoricalSeason())return toast("El histórico es solo de consulta");
  if(!eventDialog||!eventForm){console.error("No se encuentra el formulario de eventos");return toast("No se encuentra el formulario de eventos")}
  try{eventForm.reset()}catch(error){console.warn("No se pudo reiniciar el formulario de evento",error)}
  try{if(eventForm.elements.id)eventForm.elements.id.value=""}catch(_){ }
  try{
  ...
  }catch(error){console.error("No se pudo abrir Nuevo evento",error);eventDialog.setAttribute("open","")}
}
window.openNewEventForm=openNewEventForm;
addEvent?.addEventListener("click",openNewEventForm);
window.editEvent=id=>{
  const ev=(events||[]).find(x=>String(x.id)===String(id));
  if(!ev)return toast("Evento no encontrado");
  try{eventForm.reset()}catch(_){ }
  Object.entries(ev).forEach(([k,v])=>{if(eventForm?.elements?.[k])eventForm.elements[k].value=v??""});
  try{if(typeof eventDialog.showModal==="function")eventDialog.showModal();else eventDialog.setAttribute("open","")}catch(_){eventDialog.setAttribute("open","")}
};
if(eventForm)eventForm.onsubmit=async event=>{
  ...
  try{
    const r=id?await sb.from("events").update(data).eq("id",id):await sb.from("events").insert(data);
    if(r.error)throw r.error;
    await logActivity(id?"actualizar":"crear","evento",id||null,`${id?"Evento actualizado":"Evento creado"}: ${data.title||data.event_type||"Sin título"}`);
    eventDialog.close();toast("Evento guardado");await loadAll();
  }catch(error){showOperationError("No se pudo guardar el evento.",error)}
};
...
function openNewStaffForm(){
  if(isHistoricalSeason()) return toast("El histórico es solo de consulta");
  if(!staffDialog||!staffForm){
    console.error("No se encuentra el formulario del cuerpo técnico");
    toast("No se encuentra el formulario del cuerpo técnico");
    return;
  }
  try{setStaffDialogReadOnly(false)}catch(error){console.warn("Modo edición técnico",error)}
  try{staffForm.reset()}catch(error){console.warn("No se pudo reiniciar el formulario técnico",error)}
  try{if(staffForm.elements.id)staffForm.elements.id.value=""}catch(error){}
  if(staffDialogTitle)staffDialogTitle.textContent="Nuevo miembro";
  if(staffDialogSubtitle)staffDialogSubtitle.textContent="Datos profesionales, contacto y equipación";
  try{setProfileImagePreview(staffFormPhotoPreview,placeholderPhoto)}catch(error){console.warn("Vista previa técnico",error)}
  if(currentStaffPhotoFile)currentStaffPhotoFile.textContent="Sin archivo nuevo";
  try{fillStaffTeams()}catch(error){console.warn("Equipos en ficha técnica",error)}
  try{prepareStaffSizeSelects()}catch(error){console.warn("Tallajes de técnico",error)}
  try{if(staffForm.elements.sized)staffForm.elements.sized.value="No"}catch(error){}
  try{if(staffForm.elements.delivered)staffForm.elements.delivered.value="No"}catch(error){}
  try{renderCustomClothingFields("staff",{})}catch(error){console.warn("Prendas personalizadas técnico",error)}
  try{
    if(staffDialog.open)staffDialog.close();
    if(typeof staffDialog.showModal==="function")staffDialog.showModal();
    else staffDialog.setAttribute("open","");
  }catch(error){
    console.error("No se pudo abrir Nuevo miembro",error);
    staffDialog.setAttribute("open","");
  }
}
window.openNewStaffForm=openNewStaffForm;
addStaff?.addEventListener("click",openNewStaffForm);
window.editStaff=async id=>{
  ...
  const historical=isHistoricalSeason();
  setStaffDialogReadOnly(false);
  staffForm.reset();
  staffDialogTitle.textContent=member.name||"Ficha del técnico";
  if(staffDialogSubtitle)staffDialogSubtitle.textContent=historical?`Temporada ${currentSeasonLabel()}`:"Datos profesionales, contacto y equipación";
  ...
  ["id","name","role","phone","email","dni","birth_date","notes"].forEach(name=>{
    if(staffForm.elements[name]) staffForm.elements[name].value=member[name]||"";
  });
  let photoUrl=placeholderPhoto;
  if(member.photo_path){
    try{photoUrl=await signedUrl(member.photo_path)||placeholderPhoto}catch(_){photoUrl=placeholderPhoto}
  }
  setProfileImagePreview(staffFormPhotoPreview,photoUrl);
  if(currentStaffPhotoFile)currentStaffPhotoFile.textContent=member.photo_path?"Fotografía guardada":"Sin fotografía guardada";
  setStaffDialogReadOnly(historical);
...
staffForm.onsubmit=async event=>{
  ...
  const id=raw.id||null;
  const staffData={name:(raw.name||"").trim(),role:raw.role||null,team_name:raw.team_name||null,phone:(raw.phone||"").trim()||null,email:(raw.email||"").trim()||null,dni:(raw.dni||"").trim()||null,birth_date:raw.birth_date||null,notes:(raw.notes||"").trim()||null};
  ...
  try{
    ...
    const staffPhotoFile=fd.get("photo_file");
    if(staffPhotoFile&&staffPhotoFile.name)staffData.photo_path=await uploadPrivate(staffPhotoFile,"staff-photos");
    let staffId=id;
...
function openPlayerRecord(id){
  ...
  playerFormEl.reset();
  if(playerDialogTitleEl)playerDialogTitleEl.textContent=`${p.name||""} ${p.surname||""}`.trim()||"Ficha del jugador";
  if(playerDialogSubtitleEl)playerDialogSubtitleEl.textContent="Datos personales, documentación y equipación";
  setProfileImagePreview(playerFormPhotoPreviewEl,signedCache[p.photo_path]||placeholderPhoto);
  ...
  renderPlayerProfilePaymentSummary(p.id);
  try{
...
function openNewPlayerForm(){
  ...
  try{playerFormEl.reset()}catch(error){console.warn("No se pudo reiniciar el formulario de jugador",error)}
  if(playerDialogTitleEl)playerDialogTitleEl.textContent="Nuevo jugador";
  if(playerDialogSubtitleEl)playerDialogSubtitleEl.textContent="Datos personales, documentación y equipación";
  try{setProfileImagePreview(playerFormPhotoPreviewEl,placeholderPhoto)}catch(error){console.warn("Vista previa de jugador",error)}
  ...
  try{preparePlayerTeamSelect()}catch(error){console.warn("Equipos en ficha de jugador",error)}
  try{preparePlayerSizeSelects()}catch(error){console.warn("Tallajes en ficha de jugador",error)}
  try{showExistingPlayerFiles(null)}catch(error){console.warn("Documentos en ficha de jugador",error)}
  try{renderPlayerProfilePaymentSummary(null)}catch(error){console.warn("Cobros en ficha de jugador",error)}
  try{
    if(playerDialogEl.open)playerDialogEl.close();
    if(typeof playerDialogEl.showModal==="function")playerDialogEl.showModal();
    else playerDialogEl.setAttribute("open","");
  }catch(error){
    console.error("No se pudo abrir Nuevo jugador",error);
    playerDialogEl.setAttribute("open","");
  }
}
window.openNewPlayerForm=openNewPlayerForm;
...
async function start(){
  ...
  await loadClubSeasons();
  if(!isTechnicalReadOnly()&&!activeClubSeason?.id){
    try{await ensureActiveClubSeasonV1027N()}
    catch(error){showOperationError("No se ha podido recuperar la temporada activa del club.",error);return}
  }
...
async function saveClubMatchCloudV27244(match,{silent=false}={}){
  ...
  try{
    ...
    if(!payload.season_id)throw new Error("No hay una temporada activa válida para guardar el partido");
...
function weeklyMatchResultText(m){
  if(!matchIsPlayed(m))return "Pendiente";
  return `${m.goals_for??0}-${m.goals_against??0}`;
}
function weeklyMatchConditionText(m){return m.venue==="away"?"Visitante":"Local"}
function weeklyPdfSafe(value){return String(value??"").replace(/[\r\n]+/g," ").trim()}
function downloadWeeklyMatchesPdf(){
  try{
    const {jsPDF}=window.jspdf||{};
    if(!jsPDF)throw new Error("El generador PDF no está disponible");
    const {start,end,matches}=weeklyMatchesContext();
    if(!matches.length)return toast("No hay partidos programados esta semana");
    const doc=new jsPDF({orientation:"landscape",unit:"mm",format:"a4"});
    const club=weeklyPdfSafe(typeof currentClubFullName==="function"?currentClubFullName():"Manager Multiclub");
    const season=weeklyPdfSafe(typeof currentSeasonLabel==="function"?currentSeasonLabel():"");
    ...
    doc.setFont("helvetica","bold");doc.setFontSize(18);doc.text(club,14,16);
    doc.setFontSize(13);doc.text("Partidos de la semana",14,24);
    doc.setFont("helvetica","normal");doc.setFontSize(10);
    doc.text(`Temporada ${season} · ${fmt(start)} - ${fmt(weekEnd)} · ${matches.length} partido${matches.length===1?"":"s"}`,14,31);
    const rows=matches.map(m=>{
      const d=new Date(m.match_date+"T12:00:00");
      const date=d.toLocaleDateString("es-ES",{weekday:"short",day:"2-digit",month:"2-digit"});
      return [
        weeklyPdfSafe(date),weeklyPdfSafe(m.match_time||"-"),weeklyPdfSafe(m.team||"Equipo"),weeklyPdfSafe(m.opponent||"Rival"),
        weeklyMatchConditionText(m),weeklyPdfSafe(m.competition||"Partido"),weeklyPdfSafe(m.round||"-"),weeklyPdfSafe(m.location||"Pendiente"),weeklyMatchResultText(m)
      ];
    });
    if(typeof doc.autoTable==="function"){
      doc.autoTable({startY:36,head:[["Fecha","Hora","Equipo","Rival","Condición","Competición","Jornada","Campo","Resultado"]],body:rows,theme:"grid",
        styles:{fontSize:8,cellPadding:2.2,valign:"middle"},headStyles:{fillColor:[11,63,117],textColor:[255,255,255],fontStyle:"bold"},alternateRowStyles:{fillColor:[246,249,253]},
        columnStyles:{0:{cellWidth:22},1:{cellWidth:16},4:{cellWidth:20},8:{cellWidth:20}}});
    }else{
      let y=39;doc.setFontSize(8);rows.forEach(row=>{doc.text(row.join(" | ").slice(0,180),14,y);y+=5;if(y>195){doc.addPage();y=15}});
    }
    const stamp=`${start.getFullYear()}-${String(start.getMonth()+1).padStart(2,"0")}-${String(start.getDate()).padStart(2,"0")}`;
    const clubSlug=club.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")||"club";
    doc.save(`${clubSlug}_partidos_semana_${stamp}.pdf`);toast("PDF semanal descargado");
  }catch(error){console.error(error);toast("No se pudo generar el PDF semanal: "+(error.message||"error"))}
}
window.downloadWeeklyMatchesPdf=downloadWeeklyMatchesPdf;
...
function renderDashboardMatches(){
  ...
  const {matches:weekly}=weeklyMatchesContext();
  const card=m=>{
...
function setupMatchesModule(){
  ...
  document.getElementById("downloadWeeklyMatchesPdf")?.addEventListener("click",downloadWeeklyMatchesPdf);
