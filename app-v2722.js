...
// ================================================================
// V27.2.2 · Gestión multitemporada con asistente de cambio anual
...
// ================================================================
const SEASON_SELECTED_KEY="cdsb_selected_season_v2722";
let clubSeasons=[];
...
function seasonTargetTeamOptions(selected){
  const names=[...new Set(sortTeamsAlpha(teams).map(t=>String(t?.name||"").trim()).filter(Boolean))];
  const current=String(selected||"").trim();
  if(current&&!names.includes(current))names.unshift(current);
  return names.map(name=>`<option value="${esc(name)}" ${name===current?"selected":""}>${esc(name)}</option>`).join("");
}
function initSeasonRolloverDraft(){
  seasonPlayerFilter="";seasonStaffFilter="";
  seasonRolloverDraft={
    step:1,
    newName:nextSeasonLabel(activeClubSeason?.name||currentSeasonLabel()),
    keepDocuments:true,
    keepSizes:true,
    players:sortByAlpha((players||[]).filter(p=>!p.deleted_at),p=>`${p.team||""} ${p.surname||""} ${p.name||""}`).map(p=>({
      id:p.id,name:`${p.name||""} ${p.surname||""}`.trim(),currentTeam:p.team||"",targetTeam:p.team||"",keep:String(p.status||"").toLowerCase()!=="baja"
    })),
...
function renderSeasonPlayerRows(){
  ...
  const visible=d.players.filter(x=>!seasonPlayerFilter||(x.currentTeam||"Sin equipo")===seasonPlayerFilter);
  return `<div class="season-wizard-toolbar">
    ...
    <div class="season-wizard-bulk"><button type="button" class="mini" data-season-player-action="keep">Marcar visibles: continúan</button><button type="button" class="mini danger-soft" data-season-player-action="drop">Marcar visibles: baja</button></div>
  </div>
  <div class="season-wizard-table-wrap"><table class="season-wizard-table"><thead><tr><th>Continúa</th><th>Jugador</th><th>Equipo ${esc(activeClubSeason?.name||"")}</th><th>Equipo ${esc(d.newName)}</th></tr></thead><tbody>
    ${visible.map(x=>`<tr class="${x.keep?"":"season-row-drop"}"><td><input type="checkbox" data-season-player-keep="${esc(x.id)}" ${x.keep?"checked":""}></td><td><strong>${esc(x.name)}</strong></td><td>${esc(x.currentTeam||"Sin equipo")}</td><td><select data-season-player-team="${esc(x.id)}" ${x.keep?"":"disabled"}>${seasonTargetTeamOptions(x.targetTeam)}</select></td></tr>`).join("")||'<tr><td colspan="4">No hay jugadores en este filtro.</td></tr>'}
  </tbody></table></div>`;
}
...
function renderSeasonWizard(){
  ...
  if(d.step===1){
    ...
    seasonWizardBody.innerHTML=`<div class="season-wizard-intro"><h3>2. Decide qué jugadores continúan</h3><p>Marca las bajas y asigna a cada jugador que continúa el equipo de ${esc(d.newName)}. El histórico conservará su equipo anterior.</p></div>
      <div class="season-wizard-stats"><span><b>${c.playersTotal}</b> actuales</span><span class="ok"><b>${c.playersKeep}</b> continúan</span><span class="bad"><b>${c.playersDrop}</b> bajas</span></div>${renderSeasonPlayerRows()}`;
  }else if(d.step===3){
...
function validateSeasonWizardStep(){
  ...
  if(d.step===2){
    ...
    if(missing.length)return toast(`Hay ${missing.length} jugador(es) sin equipo de destino`),false;
  }
  if(d.step===3){
...
function seasonWizardVisiblePlayers(){
  ...
  return seasonRolloverDraft.players.filter(x=>!seasonPlayerFilter||(x.currentTeam||"Sin equipo")===seasonPlayerFilter);
}
...
seasonWizardBody?.addEventListener("change",event=>{
  ...
  if(t.dataset.seasonPlayerKeep){const x=d.players.find(v=>String(v.id)===String(t.dataset.seasonPlayerKeep));if(x)x.keep=t.checked;renderSeasonWizard();return}
  if(t.dataset.seasonPlayerTeam){const x=d.players.find(v=>String(v.id)===String(t.dataset.seasonPlayerTeam));if(x)x.targetTeam=t.value;return}
...
seasonWizardBody?.addEventListener("click",event=>{
  ...
  if(playerAction){seasonWizardVisiblePlayers().forEach(x=>x.keep=playerAction==="keep");renderSeasonWizard();return}
...
seasonWizardNext?.addEventListener("click",async()=>{
  const d=seasonRolloverDraft;if(!d||d.executing)return;
  if(d.step<4){if(!validateSeasonWizardStep())return;d.step++;renderSeasonWizard();return}
...
async function createFullBackup(){
  ...
  try{
    ...
    const backup={application:"CD San Bernabé Manager",version:"V27.2.2",created_at:new Date().toISOString(),tables:{}};
