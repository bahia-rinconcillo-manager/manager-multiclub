// V1.0.29 · Papelera con eliminación definitiva sobre V1.0.28 consolidada
// V1.0.27F · Visor histórico de jugador independiente del formulario de edición
...
/* V1.0.42 · imágenes privadas bajo demanda */
let mediaClubIdV1042="";
let clothingImagesLoadedV1042=false;
let clothingImagesLoadingV1042=null;
let sponsorshipLogosLoadedV1042=false;
let sponsorshipLogosLoadingV1042=null;
let mediaSignedReadsV1042=0;
function resetMediaClubV1042(){
  const club=String(typeof activeClubId==="function"?activeClubId():"");
  if(mediaClubIdV1042&&club&&club!==mediaClubIdV1042){
    clothingImages={};
    sponsorshipLogoUrls={};
    clothingImagesLoadedV1042=false;
    clothingImagesLoadingV1042=null;
    sponsorshipLogosLoadedV1042=false;
    sponsorshipLogosLoadingV1042=null;
  }
  if(club)mediaClubIdV1042=club;
}
async function ensureSponsorshipLogoV1042(sponsor){
  if(!sponsor?.logo_path)return null;
  if(sponsorshipLogoUrls[sponsor.id])return sponsorshipLogoUrls[sponsor.id];
  try{
    mediaSignedReadsV1042++;
    const url=await signedUrl(sponsor.logo_path);
    if(url)sponsorshipLogoUrls[sponsor.id]=url;
    return url||null;
  }catch(error){
    console.warn("Logo patrocinador",error);
    return null;
  }
}
async function ensureSponsorshipLogosV1042({force=false}={}){
  resetMediaClubV1042();
  if(sponsorshipLogosLoadedV1042&&!force)return true;
  if(sponsorshipLogosLoadingV1042&&!force)return sponsorshipLogosLoadingV1042;
  sponsorshipLogosLoadingV1042=(async()=>{
    if(force)sponsorshipLogoUrls={};
    await Promise.allSettled((sponsorships||[]).filter(s=>s.logo_path).map(ensureSponsorshipLogoV1042));
    sponsorshipLogosLoadedV1042=true;
    try{renderSponsorships()}catch(_){}
    return true;
  })().finally(()=>{sponsorshipLogosLoadingV1042=null});
  return sponsorshipLogosLoadingV1042;
}
window.MULTICLUB_MEDIA_V1042=Object.freeze({
  active:true,
  loadClothing:()=>loadClothingImages(),
  loadSponsorships:()=>ensureSponsorshipLogosV1042(),
  metrics:()=>({
    clubId:mediaClubIdV1042,
    clothingLoaded:clothingImagesLoadedV1042,
    sponsorshipsLoaded:sponsorshipLogosLoadedV1042,
    signedReads:mediaSignedReadsV1042
  })
});
// ============================================================================
...
const MULTICLUB_BUILD_VERSION="V1.0.42";


// V1.0.27G — Visor histórico robusto. Se instala al principio del script para
// interceptar "Abrir ficha" antes que cualquier bloqueo general de solo lectura.
function historyEscapeV1027G(value){
  return String(value??"").replace(/[&<>"']/g,ch=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[ch]));
}
function historyValueV1027G(value,fallback="—"){
  const text=String(value??"").trim();
  return text?historyEscapeV1027G(text):fallback;
}
function historyDateV1027G(value){
  const text=String(value??"").trim();
  if(!text)return "—";
  const parts=text.slice(0,10).split("-");
  if(parts.length===3&&parts[0].length===4)return `${parts[2]}/${parts[1]}/${parts[0]}`;
  return historyEscapeV1027G(text);
}
function historyEuroV1027G(value){
  const n=Number(value||0);
  try{return n.toLocaleString("es-ES",{minimumFractionDigits:2,maximumFractionDigits:2})+" €"}catch(_){return `${n.toFixed(2)} €`}
}
function closeHistoricalPlayerOverlayV1027G(){
  const overlay=document.getElementById("historicalPlayerOverlayV1027G");
  if(overlay)overlay.hidden=true;
  document.body?.classList.remove("historical-player-overlay-open-v1027g");
}
function openHistoricalPlayerOverlayV1027G(id){
  try{
    const player=(players||[]).find(row=>String(row?.id)===String(id));
    if(!player){
      const msg="No se encontró el jugador dentro de la copia histórica de esta temporada.";
      if(typeof toast==="function")toast(msg);else alert(msg);
      return;
    }
    const overlay=document.getElementById("historicalPlayerOverlayV1027G");
    const body=document.getElementById("historicalPlayerOverlayBodyV1027G");
    const title=document.getElementById("historicalPlayerOverlayTitleV1027G");
    const season=document.getElementById("historicalPlayerOverlaySeasonV1027G");
    if(!overlay||!body)throw new Error("No se encuentra el visor histórico V1.0.27G en index.html");

    const doc=(documents||[]).find(row=>String(row?.player_id)===String(player.id))||{};
    const kitRow=(kits||[]).find(row=>String(row?.player_id)===String(player.id))||{};
    const sizeRow=(playerSizes||[]).find(row=>String(row?.player_id)===String(player.id))||{};
    const customSizes=(typeof customPlayerClothingSizes!=="undefined"&&customPlayerClothingSizes?.[player.id])||{};
    const sizes={...sizeRow,...customSizes};
    const payRows=(payments||[]).filter(row=>String(row?.player_id)===String(player.id));
    const conceptName=key=>{
      const c=(paymentConcepts||[]).find(row=>String(row?.concept_key)===String(key));
      return c?.name||key||"Concepto";
    };
    const paidTotal=payRows.reduce((sum,row)=>sum+Number(row?.amount||0),0);
    let pendingTotal=0;
    for(const concept of (paymentConcepts||[])){
      if(concept?.active===false||concept?.required===false)continue;
      const row=payRows.find(item=>String(item?.concept)===String(concept?.concept_key));
      if(row?.status==="Exento")continue;
      pendingTotal+=Math.max(0,Number(concept?.default_amount||0)-Number(row?.amount||0));
    }
    const photo=(signedCache&&player.photo_path&&signedCache[player.photo_path])||"assets/avatar-jugador.svg";
    const sizeLabels=[
      ["Camiseta 1ª jugador",sizes.game_shirt],["Camiseta 1ª portero",sizes.game_shirt_goalkeeper],
      ["Pantalón jugador",sizes.game_shorts],["Pantalón portero",sizes.game_shorts_goalkeeper],
      ["Camiseta 2ª jugador",sizes.second_shirt_player],["Medias jugador",sizes.socks],
      ["Medias portero",sizes.socks_goalkeeper],["Camiseta entrenamiento jugador",sizes.training_shirt],
      ["Camiseta entrenamiento portero",sizes.training_shirt_goalkeeper],["Pantalón entrenamiento jugador",sizes.training_shorts],
      ["Pantalón entrenamiento portero",sizes.training_shorts_goalkeeper],["Sudadera entrenamiento",sizes.training_sweatshirt],
      ["Chaqueta chándal",sizes.tracksuit_jacket],["Pantalón chándal",sizes.tracksuit_trousers]
    ].filter(([,value])=>String(value??"").trim());

    if(title)title.textContent=`${player.name||""} ${player.surname||""}`.trim()||"Ficha del jugador";
    if(season)season.textContent=`Temporada ${typeof currentSeasonLabel==="function"?currentSeasonLabel():"histórica"} · SOLO LECTURA`;
    body.innerHTML=`
      <section class="histg-card histg-summary">
        <img class="histg-photo" src="${historyEscapeV1027G(photo)}" alt="Foto del jugador" onerror="this.src='assets/avatar-jugador.svg'">
        <div class="histg-grid">
          <div><span>Nombre</span><strong>${historyValueV1027G(player.name)}</strong></div>
          <div><span>Apellidos</span><strong>${historyValueV1027G(player.surname)}</strong></div>
          <div><span>Fecha de nacimiento</span><strong>${historyDateV1027G(player.birth_date)}</strong></div>
          <div><span>DNI/NIE</span><strong>${historyValueV1027G(player.player_dni)}</strong></div>
          <div><span>Equipo</span><strong>${historyValueV1027G(player.team,"Sin equipo")}</strong></div>
          <div><span>Estado</span><strong>${historyValueV1027G(player.status)}</strong></div>
        </div>
      </section>
      <section class="histg-card"><h3>Familia y contacto</h3><div class="histg-grid">
        <div><span>Tutor</span><strong>${historyValueV1027G(player.guardian)}</strong></div>
        <div><span>Teléfono</span><strong>${historyValueV1027G(player.phone)}</strong></div>
        <div><span>Email</span><strong>${historyValueV1027G(player.email)}</strong></div>
        <div><span>Dirección</span><strong>${historyValueV1027G(player.address)}</strong></div>
        <div class="histg-wide"><span>Observaciones</span><strong>${historyValueV1027G(player.notes)}</strong></div>
      </div></section>
      <section class="histg-card"><h3>Documentación</h3><div class="histg-grid">
        <div><span>DNI / libro de familia</span><strong>${historyValueV1027G(doc.player_dni_status||"Pendiente")}</strong></div>
        <div><span>Fotografía</span><strong>${historyValueV1027G(doc.photo_status||"Pendiente")}</strong></div>
        <div><span>Reconocimiento médico</span><strong>${historyValueV1027G(doc.medical_status||"Pendiente")}</strong></div>
      </div></section>
      <section class="histg-card"><h3>Cobros</h3>
        ${payRows.length?`<div class="histg-payments">${payRows.map(row=>`<div><span>${historyEscapeV1027G(conceptName(row.concept))}</span><strong>${historyValueV1027G(row.status||"Pendiente")}</strong><b>${historyEuroV1027G(row.amount)}</b></div>`).join("")}</div>`:'<p class="histg-empty">No hay registros de cobro en esta temporada.</p>'}
        <div class="histg-totals"><span>Pagado <strong>${historyEuroV1027G(paidTotal)}</strong></span><span>Pendiente <strong>${historyEuroV1027G(pendingTotal)}</strong></span></div>
      </section>
      <section class="histg-card"><h3>Tallajes</h3>
        ${sizeLabels.length?`<div class="histg-grid">${sizeLabels.map(([label,value])=>`<div><span>${historyEscapeV1027G(label)}</span><strong>${historyValueV1027G(value)}</strong></div>`).join("")}</div>`:'<p class="histg-empty">No hay tallajes guardados en esta temporada.</p>'}
      </section>
      <section class="histg-card"><h3>Equipación</h3><div class="histg-grid">
        <div><span>Estado de entrega</span><strong>${historyValueV1027G(kitRow.delivered||"Pendiente")}</strong></div>
        <div><span>Fecha de entrega</span><strong>${historyDateV1027G(kitRow.delivered_at||kitRow.delivery_date)}</strong></div>
      </div></section>`;
    overlay.hidden=false;
    document.body?.classList.add("historical-player-overlay-open-v1027g");
    body.scrollTop=0;
  }catch(error){
    console.error("V1.0.27G · Error abriendo ficha histórica",error);
    const msg=`No se pudo abrir la ficha histórica: ${error?.message||error}`;
    if(typeof toast==="function")toast(msg);else alert(msg);
  }
}
window.openHistoricalPlayerOverlayV1027G=openHistoricalPlayerOverlayV1027G;
window.closeHistoricalPlayerOverlayV1027G=closeHistoricalPlayerOverlayV1027G;

// Captura temprana: funciona incluso si otra capa de solo lectura desactiva
// posteriormente los manejadores normales del formulario de jugador.
document.addEventListener("click",event=>{
  const close=event.target.closest?.("[data-history-overlay-close-v1027g]");
  if(close){event.preventDefault();event.stopImmediatePropagation();closeHistoricalPlayerOverlayV1027G();return;}
  const target=event.target.closest?.("[data-open-player-record]");
  if(!target||!(typeof isHistoricalSeason==="function"&&isHistoricalSeason()))return;
  event.preventDefault();
  event.stopImmediatePropagation();
  openHistoricalPlayerOverlayV1027G(target.dataset.openPlayerRecord);
},true);
...
/* V1.0.35 · fotos privadas solo cuando van a mostrarse */
const lazyPhotoPendingV1035=new Map();
let lazyPhotoObserverV1035=null;
async function ensurePhotoUrlV1035(path){
  const key=String(path||"");
  if(!key)return placeholderPhoto;
  if(signedCache[key])return signedCache[key];
  if(lazyPhotoPendingV1035.has(key))return lazyPhotoPendingV1035.get(key);
  const task=Promise.resolve(signedUrl(key)).then(url=>{
    if(url)signedCache[key]=url;
    return url||placeholderPhoto;
  }).catch(()=>placeholderPhoto).finally(()=>lazyPhotoPendingV1035.delete(key));
  lazyPhotoPendingV1035.set(key,task);
  return task;
}
async function hydratePhotoNodeV1035(img){
  if(!img||img.dataset.photoReadyV1035==="1")return;
  const path=String(img.dataset.photoPath||"");
  if(!path){img.dataset.photoReadyV1035="1";return}
  img.dataset.photoReadyV1035="loading";
  const url=await ensurePhotoUrlV1035(path);
  if(img.isConnected&&String(img.dataset.photoPath||"")===path){
    img.src=url||placeholderPhoto;
    img.dataset.photoReadyV1035="1";
  }
}
function scanLazyPhotosV1035(root=document){
  const nodes=[];
  if(root?.matches?.("img[data-photo-path]"))nodes.push(root);
  root?.querySelectorAll?.("img[data-photo-path]")?.forEach(img=>nodes.push(img));
  if(!nodes.length)return;
  if("IntersectionObserver" in window){
    if(!lazyPhotoObserverV1035)lazyPhotoObserverV1035=new IntersectionObserver(entries=>{
      for(const entry of entries){
        if(!entry.isIntersecting)continue;
        lazyPhotoObserverV1035.unobserve(entry.target);
        hydratePhotoNodeV1035(entry.target);
      }
    },{rootMargin:"320px 0px"});
    nodes.forEach(img=>{
      if(img.dataset.photoReadyV1035!=="1"&&img.dataset.photoReadyV1035!=="loading")lazyPhotoObserverV1035.observe(img);
    });
  }else nodes.forEach(hydratePhotoNodeV1035);
}
window.scanMulticlubPhotosV1035=scanLazyPhotosV1035;
new MutationObserver(entries=>{
  for(const entry of entries)for(const node of entry.addedNodes)if(node?.nodeType===1)scanLazyPhotosV1035(node);
...
async function loadClothingImages({force=false}={}){
  resetMediaClubV1042();
  if(clothingImagesLoadedV1042&&!force)return true;
  if(clothingImagesLoadingV1042&&!force)return clothingImagesLoadingV1042;
  clothingImagesLoadingV1042=(async()=>{
    if(force)clothingImages={};
    try{
      ...
      clothingImagesLoadedV1042=true;
      try{renderClothingCatalog();refreshKitDialogImages()}catch(_){}
      return true;
    }catch(error){
      console.warn("No se pudieron cargar las fotos de ropa",error);
      return false;
    }finally{
      clothingImagesLoadingV1042=null;
    }
  })();
  return clothingImagesLoadingV1042;
}
...
// V1.0.28 · Mejoras visuales de ficha sin sustituir los manejadores estables.
const playerFormPhotoPreviewEl=getEl("playerFormPhotoPreview");
const playerFormIdentityTextEl=getEl("playerFormIdentityText");
function setPlayerProfilePreview(src){
  if(!playerFormPhotoPreviewEl)return;
  playerFormPhotoPreviewEl.src=src||placeholderPhoto;
  playerFormPhotoPreviewEl.onerror=()=>{playerFormPhotoPreviewEl.onerror=null;playerFormPhotoPreviewEl.src=placeholderPhoto};
}
function renderPlayerFormPaymentSummaryV1028(playerId){
  const box=getEl("playerFormPaymentSummary");if(!box)return;
  if(!playerId){box.className="profile-payment-empty";box.innerHTML="Los cobros aparecerán después de guardar el jugador.";return;}
  const rows=(payments||[]).filter(row=>String(row?.player_id)===String(playerId));
  const label=key=>paymentConcepts?.find(c=>String(c?.concept_key)===String(key))?.name||key||"Concepto";
  const paid=rows.reduce((sum,row)=>sum+Number(row?.amount||0),0);
  let due=0;
  for(const concept of (paymentConcepts||[])){
    if(concept?.active===false||concept?.required===false)continue;
    const row=rows.find(item=>String(item?.concept)===String(concept?.concept_key));
    if(row?.status==="Exento")continue;
    due+=Math.max(0,Number(concept?.default_amount||0)-Number(row?.amount||0));
  }
  box.className="";
  box.innerHTML=(rows.length?rows.map(row=>`<div class="profile-payment-row"><span>${esc(label(row.concept))}</span><strong>${esc(row.status||"Pendiente")}</strong><b>${euro(row.amount||0)}</b></div>`).join(""):'<div class="profile-payment-empty">No hay cobros registrados.</div>')+`<div class="profile-payment-total"><span>Pagado <strong>${euro(paid)}</strong></span><span>Pendiente <strong>${euro(due)}</strong></span></div>`;
}
playerFormEl?.elements?.photo_file?.addEventListener("change",event=>{
  const file=event.currentTarget?.files?.[0];if(!file)return;
  const url=URL.createObjectURL(file);setPlayerProfilePreview(url);
  playerFormPhotoPreviewEl?.addEventListener("load",()=>URL.revokeObjectURL(url),{once:true});
});
...
let sportsLoadedV1032=false;
let sportsLoadPromiseV1032=null;
async function ensureSportsLoadedV1032(){
  if(sportsLoadedV1032)return true;
  if(sportsLoadPromiseV1032)return sportsLoadPromiseV1032;
  const api=window.sportsV2703||window.sportsV2702||window.sportsV2701||window.sportsV2700;
  if(!api?.load)return false;
  sportsLoadPromiseV1032=Promise.resolve(api.load()).then(result=>{
    sportsLoadedV1032=result!==false;
    return sportsLoadedV1032;
  }).catch(error=>{
    console.warn("Multiclub V1.0.32 · Deportes bajo demanda",error);
    return false;
  }).finally(()=>{sportsLoadPromiseV1032=null});
  return sportsLoadPromiseV1032;
}
window.ensureSportsLoadedV1032=ensureSportsLoadedV1032;

async function loadActiveSeasonData(){
  if(typeof loadClubMatches==="function")loadClubMatches();
  await loadMulticlubEquipmentState();
  await loadAll();
  sportsLoadedV1032=false;
}
...
function applySeasonReadOnlyUi(){
  ...
  document.querySelectorAll('[data-season-readonly-allow="true"]').forEach(button=>{
    if(button.tagName==="BUTTON")button.disabled=false;
    button.removeAttribute("data-season-readonly-disabled");
    if(button.title==="La temporada histórica es de solo lectura")button.removeAttribute("title");
  });
  ...
  document.querySelectorAll("#app button, dialog button").forEach(button=>{
    if(button.matches("[data-season-open],[data-season-readonly-allow='true'],.nav,#rPrint,[data-close]")||button.closest("#seasons")||button.disabled)return;
...
document.addEventListener("click",event=>{
  ...
  if(button.matches("[data-season-open],[data-season-readonly-allow='true'],.nav,#rPrint,[data-close],[data-report-export]")||button.closest("#seasons"))return;
  const text=(button.textContent||"").trim();
...
const ACTIVITY_COLUMNS_V1035="id,action,description,user_email,created_at";
const TRASH_PLAYER_COLUMNS_V1035="id,name,surname,team,deleted_at";
const TRASH_TEAM_COLUMNS_V1035="id,name,category,deleted_at";
let activityLoadedFullV1035=false;
let trashLoadedV1035=false;

async function refreshActivityV1035(limit=6){
  const safeLimit=Math.max(1,Math.min(150,Number(limit)||6));
  const {data,error}=await sb.from("activity_logs")
    .select(ACTIVITY_COLUMNS_V1035)
    .eq("club_id",activeClubId())
    .order("created_at",{ascending:false})
    .limit(safeLimit);
  if(error)throw error;
  activityLogs=data||[];
  activityLoadedFullV1035=safeLimit>=150;
  try{renderActivity();renderDashboardExtras()}catch(_){}
}
async function refreshTrashV1035(){
  if(!activeClubSeason?.id)return;
  const [p,t]=await Promise.all([
    sb.from("players").select(TRASH_PLAYER_COLUMNS_V1035).not("deleted_at","is",null).order("deleted_at",{ascending:false}),
    sb.from("teams").select(TRASH_TEAM_COLUMNS_V1035).eq("season_id",activeClubSeason.id).not("deleted_at","is",null).order("deleted_at",{ascending:false})
  ]);
  if(p.error)throw p.error;
  if(t.error)throw t.error;
  deletedPlayers=p.data||[];
  deletedTeams=t.data||[];
  trashLoadedV1035=true;
  try{renderTrash()}catch(_){}
}
async function loadTrashAndActivity(){
  const tasks=[];
  if(document.getElementById("trash")?.classList.contains("active"))tasks.push(refreshTrashV1035());
  if(document.getElementById("activity")?.classList.contains("active"))tasks.push(refreshActivityV1035(150));
  else if(document.getElementById("dashboard")?.classList.contains("active"))tasks.push(refreshActivityV1035(6));
...
document.addEventListener("click",event=>{
  ...
  if(view==="trash"&&!trashLoadedV1035)refreshTrashV1035().catch(error=>console.warn("Papelera",error));
  if(view==="activity"&&!activityLoadedFullV1035)refreshActivityV1035(150).catch(error=>console.warn("Actividad",error));
  if(view==="dashboard"&&!activityLogs.length)refreshActivityV1035(6).catch(error=>console.warn("Actividad Inicio",error));
},true);
...
const EVENT_COLUMNS_V1040="id,title,event_type,event_date,event_time,team_name,location,description";
const EVENTS_DASHBOARD_LIMIT_V1040=5;
let eventsFullLoadedV1040=false;
let eventsFullLoadingV1040=null;
let eventsSeasonIdV1040="";
let eventsStartupReadsV1040=0;
let eventsFullReadsV1040=0;

function todayIsoV1040(){return new Date().toISOString().slice(0,10)}
function resetEventsStateV1040(){
  const seasonId=String(activeClubSeason?.id||"");
  if(eventsSeasonIdV1040&&seasonId&&eventsSeasonIdV1040!==seasonId){
    eventsFullLoadedV1040=false;
    eventsFullLoadingV1040=null;
  }
  if(seasonId)eventsSeasonIdV1040=seasonId;
}
function dashboardEventsQueryV1040(){
  eventsStartupReadsV1040++;
  return sb.from("events").select(EVENT_COLUMNS_V1040)
    .gte("event_date",todayIsoV1040())
    .order("event_date")
    .limit(EVENTS_DASHBOARD_LIMIT_V1040);
}
async function ensureFullEventsV1040({force=false}={}){
  if(isTechnicalReadOnly())return false;
  resetEventsStateV1040();
  if(eventsFullLoadedV1040&&!force)return true;
  if(eventsFullLoadingV1040&&!force)return eventsFullLoadingV1040;
  eventsFullLoadingV1040=(async()=>{
    try{
      eventsFullReadsV1040++;
      const {data,error}=await sb.from("events").select(EVENT_COLUMNS_V1040).order("event_date");
      if(error)throw error;
      events=Array.isArray(data)?data:[];
      eventsFullLoadedV1040=true;
      try{renderUpcomingEvents()}catch(_){}
      try{renderCalendar()}catch(_){}
      return true;
    }catch(error){
      console.warn("Manager Multiclub V1.0.40 · agenda completa",error);
      return false;
    }finally{
      eventsFullLoadingV1040=null;
    }
  })();
  return eventsFullLoadingV1040;
}
document.addEventListener("click",event=>{
  const nav=event.target.closest?.('.nav[data-view="calendar"]');
  const direct=event.target.closest?.('[data-dashboard-go="calendar"],[data-go="calendar"]');
  if(!nav&&!direct)return;
  ensureFullEventsV1040().catch(error=>console.warn("Agenda completa",error));
},true);
window.MULTICLUB_EVENTS_V1040=Object.freeze({
  active:true,
  dashboardLimit:EVENTS_DASHBOARD_LIMIT_V1040,
  loadFull:ensureFullEventsV1040,
  fullLoaded:()=>eventsFullLoadedV1040,
  metrics:()=>({startupReads:eventsStartupReadsV1040,fullReads:eventsFullReadsV1040,fullLoaded:eventsFullLoadedV1040,seasonId:eventsSeasonIdV1040})
});
...
async function loadAll(){
  ...
  resetEventsStateV1040();
  const eventsQueryV1040=eventsFullLoadedV1040
    ? sb.from("events").select(EVENT_COLUMNS_V1040).order("event_date")
    : dashboardEventsQueryV1040();
  const queries=await Promise.allSettled([
    sb.from("players").select("id,name,surname,birth_date,player_dni,team,guardian,phone,email,address,notes,status,photo_path,created_at,deleted_at").is("deleted_at",null).order("created_at",{ascending:false}),
    sb.from("payments").select("id,player_id,concept,status,amount"),
    sb.from("payment_concepts").select("*").order("sort_order").order("name"),
    sb.from("documents").select("id,player_id,player_dni_status,guardian_dni_status,photo_status,medical_status,player_dni_path,guardian_dni_path,photo_path,medical_path"),
    sb.from("kits").select("id,player_id,sized,delivered,delivery_date"),
    sb.from("teams").select("id,name,age_category,category,coach_id,delegate_name,training_schedule,field,notes,season_id,created_at,deleted_at").eq("season_id",activeClubSeason.id).is("deleted_at",null).order("name"),
    sb.from("staff").select("id,name,role,team_name,phone,email,notes,photo_path,created_at").order("name"),
    eventsQueryV1040,
  ...
  // V1.0.42: logos de patrocinio y fotos del catálogo se firman solo al entrar
  // en sus pantallas. Así el arranque no descarga imágenes privadas innecesarias.
  sponsorshipLogoUrls={};
  sponsorshipLogosLoadedV1042=false;
  sponsorshipLogosLoadingV1042=null;
  await loadTrashAndActivity();
...
document.addEventListener("click",event=>{
  const nav=event.target.closest?.(".nav[data-view]");
  if(nav?.dataset.view==="kits"){
    loadClothingImages().catch(error=>console.warn("Fotos equipaciones",error));
  }
  if(nav?.dataset.view==="sponsorships"){
    ensureSponsorshipLogosV1042().catch(error=>console.warn("Logos patrocinio",error));
  }
},true);
document.addEventListener("DOMContentLoaded",()=>{
  ["playerDialog","staffDialog","kitDialog"].forEach(id=>{
    const dialog=document.getElementById(id);
    if(!dialog)return;
    new MutationObserver(()=>{
      if(dialog.open)loadClothingImages().catch(error=>console.warn("Fotos equipaciones",error));
...
function render(){
  ...
  setTimeout(()=>scanLazyPhotosV1035(document),0);
}
...
function renderPlayers(){
  ...
  grid.innerHTML=list.length?list.map(p=>{
    ...
    return `<article class="player-professional-card" data-open-player-record="${p.id}" data-season-readonly-allow="true">
      <div class="player-photo-wrap">
        <img src="${photo}" data-photo-path="${esc(p.photo_path||"")}" alt="Foto de ${esc(p.name||"jugador")}" onerror="this.src='${placeholderPhoto}'">
      ...
      <div class="player-card-main">
        ...
        <div class="player-card-footer">
          ...
          ${isHistoricalSeason()?`<a href="#" class="mini" role="button" data-open-player-record="${p.id}" data-season-readonly-allow="true">Abrir ficha</a>`:`<button class="mini" type="button" data-open-player-record="${p.id}" data-season-readonly-allow="true">Abrir ficha</button>`}
        </div>
      </div>
    </article>`;
  ...
  tableBody.innerHTML=list.length?list.map(p=>{
    const photo=signedCache[p.photo_path]||placeholderPhoto;
    return `<tr>
      <td><img class="player-table-photo" src="${photo}" data-photo-path="${esc(p.photo_path||"")}" onerror="this.src='${placeholderPhoto}'"></td>
      ...
      <td>${isHistoricalSeason()?`<a href="#" class="mini" role="button" data-open-player-record="${p.id}" data-season-readonly-allow="true">Abrir</a>`:`<button class="mini" type="button" data-open-player-record="${p.id}" data-season-readonly-allow="true">Abrir</button>`}</td>
    </tr>`;
...
teamForm.onsubmit=async event=>{
  ...
  try{
    if(!activeClubSeason?.id)await loadClubSeasons();
    const seasonId=activeClubSeason?.id||null;
    if(!seasonId)throw new Error("No se ha podido identificar la temporada activa. Recarga la aplicación antes de guardar el equipo.");
    const data={name:(raw.name||"").trim(),age_category:(raw.age_category||"").trim()||null,category:(raw.category||"").trim()||null,coach_id:raw.coach_id||null,delegate_name:(raw.delegate_name||"").trim()||null,training_schedule:(raw.training_schedule||"").trim()||null,field:(raw.field||"").trim()||null,notes:(raw.notes||"").trim()||null,season_id:seasonId};
...
window.openSponsorship=id=>{
  ...
  if(item?.logo_path&&!sponsorshipLogoUrls[item.id]){
    ensureSponsorshipLogoV1042(item).then(url=>{
      if(!url||!sponsorshipDialog?.open)return;
      const current=String(sponsorshipForm?.elements?.id?.value||"");
      if(current===String(item.id))showSponsorshipPreview(url);
    });
  }
};
...
function ensureHistoricalPlayerViewer(){
  let dialog=document.getElementById("historicalPlayerViewer");
  if(dialog)return dialog;
  dialog=document.createElement("dialog");
  dialog.id="historicalPlayerViewer";
  dialog.className="historical-player-viewer";
  dialog.innerHTML=`<div class="historical-player-viewer-shell">
    <div class="historical-player-viewer-head">
      <div><small>JUGADORES · HISTÓRICO</small><h2 id="historicalPlayerViewerTitle">Ficha del jugador</h2><p id="historicalPlayerViewerSeason"></p></div>
      <button type="button" class="historical-player-viewer-close" data-history-player-close aria-label="Cerrar">×</button>
    </div>
    <div id="historicalPlayerViewerBody" class="historical-player-viewer-body"></div>
    <div class="historical-player-viewer-actions"><button type="button" class="secondary" data-history-player-close>Cerrar</button></div>
  </div>`;
  document.body.appendChild(dialog);
  dialog.addEventListener("click",event=>{
    const close=event.target.closest?.("[data-history-player-close]");
    if(close){event.preventDefault();try{dialog.close()}catch(_){dialog.removeAttribute("open")}return;}
    if(event.target===dialog){try{dialog.close()}catch(_){dialog.removeAttribute("open")}}
  });
  return dialog;
}
function historyViewerValue(value,fallback="—"){
  const text=String(value??"").trim();
  return text?esc(text):fallback;
}
function historicalPaymentRows(playerId){
  const rows=payments.filter(row=>String(row.player_id)===String(playerId));
  if(!rows.length)return '<div class="historical-viewer-empty">No hay registros de cobro guardados en esta temporada.</div>';
  return `<div class="historical-viewer-table">${rows.map(row=>`<div class="historical-viewer-table-row"><span>${esc(paymentConceptLabel(row.concept))}</span><strong>${esc(row.status||"Pendiente")}</strong><span>${euro(row.amount||0)}</span></div>`).join("")}</div>`;
}
function historicalDocumentRow(playerId,kind,label){
  const asset=playerDocumentAsset(playerId,kind);
  return `<div class="historical-viewer-doc"><span>${esc(label)}</span>${asset.path?`<div class="historical-viewer-doc-actions"><button type="button" class="mini" data-history-doc-view="${esc(kind)}" data-history-player-id="${esc(playerId)}">Ver</button><button type="button" class="mini" data-history-doc-download="${esc(kind)}" data-history-player-id="${esc(playerId)}">Descargar</button></div>`:'<strong>Sin archivo adjunto</strong>'}</div>`;
}
function openHistoricalPlayerViewer(id){
  const p=players.find(x=>String(x.id)===String(id));
  if(!p)return toast("Jugador no encontrado en esta temporada");
  const d=docs(p.id);
  const sizes=sizeFor(p.id)||{};
  const k=kit(p.id)||{};
  const dialog=ensureHistoricalPlayerViewer();
  const title=dialog.querySelector("#historicalPlayerViewerTitle");
  const season=dialog.querySelector("#historicalPlayerViewerSeason");
  const body=dialog.querySelector("#historicalPlayerViewerBody");
  if(title)title.textContent=`${p.name||""} ${p.surname||""}`.trim()||"Ficha del jugador";
  if(season)season.textContent=`Temporada ${currentSeasonLabel()} · SOLO LECTURA`;
  const photo=signedCache[p.photo_path]||placeholderPhoto;
  const sizeFields=[
    ["Camiseta 1ª jugador",sizes.game_shirt],["Camiseta 1ª portero",sizes.game_shirt_goalkeeper],
    ["Pantalón jugador",sizes.game_shorts],["Pantalón portero",sizes.game_shorts_goalkeeper],
    ...
    ["Camiseta entrenamiento portero",sizes.training_shirt_goalkeeper],["Pantalón entrenamiento jugador",sizes.training_shorts],
    ["Pantalón entrenamiento portero",sizes.training_shorts_goalkeeper],["Sudadera entrenamiento",sizes.training_sweatshirt],
    ["Chaqueta chándal",sizes.tracksuit_jacket],["Pantalón chándal",sizes.tracksuit_trousers],["Mochila",sizes.backpack||"Incluida"]
  ].filter(([,value])=>String(value??"").trim());
  body.innerHTML=`
    <div class="historical-viewer-summary">
      <img src="${photo}" alt="Foto del jugador" onerror="this.src='${placeholderPhoto}'">
      <div class="historical-viewer-summary-data">
        <div><span>Nombre</span><strong>${historyViewerValue(p.name)}</strong></div>
        <div><span>Apellidos</span><strong>${historyViewerValue(p.surname)}</strong></div>
        <div><span>Fecha de nacimiento</span><strong>${historyViewerValue(reportDate(p.birth_date))}</strong></div>
        <div><span>DNI/NIE</span><strong>${historyViewerValue(p.player_dni)}</strong></div>
        <div><span>Equipo</span><strong>${historyViewerValue(p.team,"Sin equipo")}</strong></div>
        <div><span>Estado</span><strong>${historyViewerValue(p.status)}</strong></div>
      </div>
    </div>
    <section class="historical-viewer-section"><h3>Familia y contacto</h3><div class="historical-viewer-grid">
      <div><span>Tutor</span><strong>${historyViewerValue(p.guardian)}</strong></div>
      <div><span>Teléfono</span><strong>${historyViewerValue(p.phone)}</strong></div>
      <div><span>Email</span><strong>${historyViewerValue(p.email)}</strong></div>
      <div class="wide"><span>Dirección</span><strong>${historyViewerValue(p.address)}</strong></div>
      <div class="wide"><span>Observaciones</span><strong>${historyViewerValue(p.notes)}</strong></div>
    </div></section>
    <section class="historical-viewer-section"><h3>Documentación</h3><div class="historical-viewer-grid">
      <div><span>DNI / libro de familia</span><strong>${historyViewerValue(d.player_dni_status||"Pendiente")}</strong></div>
      <div><span>Fotografía</span><strong>${historyViewerValue(d.photo_status||"Pendiente")}</strong></div>
      <div><span>Reconocimiento médico</span><strong>${historyViewerValue(d.medical_status||"Pendiente")}</strong></div>
    </div><div class="historical-viewer-docs">
      ${historicalDocumentRow(p.id,"dni","DNI / libro de familia")}
      ${historicalDocumentRow(p.id,"photo","Fotografía")}
      ${historicalDocumentRow(p.id,"medical","Reconocimiento médico")}
    </div></section>
    <section class="historical-viewer-section"><h3>Cobros</h3>${historicalPaymentRows(p.id)}<div class="historical-viewer-total"><span>Total pagado</span><strong>${euro(totalPaid(p.id))}</strong><span>Pendiente</span><strong>${euro(pending(p.id))}</strong></div></section>
    <section class="historical-viewer-section"><h3>Ropa y tallaje</h3>${sizeFields.length?`<div class="historical-viewer-grid">${sizeFields.map(([label,value])=>`<div><span>${esc(label)}</span><strong>${historyViewerValue(value)}</strong></div>`).join("")}</div>`:'<div class="historical-viewer-empty">No hay tallajes guardados.</div>'}</section>
    <section class="historical-viewer-section"><h3>Equipación</h3><div class="historical-viewer-grid"><div><span>Estado de entrega</span><strong>${historyViewerValue(k.delivered||"Pendiente")}</strong></div><div><span>Fecha de entrega</span><strong>${historyViewerValue(reportDate(k.delivered_at||k.delivery_date))}</strong></div></div></section>`;
  body.querySelectorAll("[data-history-doc-view]").forEach(button=>button.addEventListener("click",()=>openPlayerDocument(button.dataset.historyPlayerId,button.dataset.historyDocView,false)));
  body.querySelectorAll("[data-history-doc-download]").forEach(button=>button.addEventListener("click",()=>openPlayerDocument(button.dataset.historyPlayerId,button.dataset.historyDocDownload,true)));
  try{
    if(dialog.open)dialog.close();
    if(typeof dialog.showModal==="function")dialog.showModal();
    else dialog.setAttribute("open","");
  }catch(error){
    console.error("No se pudo abrir el visor histórico del jugador",error);
    dialog.setAttribute("open","");
  }
}
window.openHistoricalPlayerViewer=openHistoricalPlayerViewer;

function openPlayerRecord(id){
  if(isHistoricalSeason()){
    openHistoricalPlayerOverlayV1027G(id);
    return;
  }
  const p=players.find(x=>String(x.id)===String(id));
  ...
  playerFormEl.reset();
  preparePlayerTeamSelect(p.team);
  preparePlayerSizeSelects(sizeFor(p.id));
  deletePlayerFromForm?.classList.remove("hidden");
  deletePlayerFromForm?.setAttribute("data-player-id",p.id);
  ["id","name","surname","birth_date","player_dni","status","guardian","phone","email","address","notes"].forEach(k=>{
    if(playerFormEl.elements[k])playerFormEl.elements[k].value=p[k]??"";
  });
  showExistingPlayerFiles(p.id);
  setPlayerProfilePreview(signedCache[p.photo_path]||placeholderPhoto);
  if(p.photo_path&&!signedCache[p.photo_path]){
    ensurePhotoUrlV1035(p.photo_path).then(url=>{
      if(String(playerFormEl?.elements?.id?.value||"")===String(p.id))setPlayerProfilePreview(url||placeholderPhoto);
    });
  }
  if(playerFormIdentityTextEl)playerFormIdentityTextEl.textContent=`${p.name||""} ${p.surname||""}`.trim()||"Ficha del jugador";
  renderPlayerFormPaymentSummaryV1028(p.id);
  try{
    if(typeof playerDialogEl.showModal==="function")playerDialogEl.showModal();
    else playerDialogEl.setAttribute("open","");
  }catch(error){
    console.error("No se pudo abrir la ficha del jugador",error);
    playerDialogEl.setAttribute("open","");
  }
}
window.openPlayerRecord=openPlayerRecord;
...
document.addEventListener("click",event=>{
  ...
  event.preventDefault();
  event.stopPropagation();
  const id=target.dataset.openPlayerRecord;
  if(id)openPlayerRecord(id);
});
...
function openNewPlayerForm(){
  ...
  setPlayerProfilePreview(placeholderPhoto);
  if(playerFormIdentityTextEl)playerFormIdentityTextEl.textContent="Nuevo jugador";
  renderPlayerFormPaymentSummaryV1028(null);
  try{if(typeof playerDialogEl.showModal==="function")playerDialogEl.showModal();else playerDialogEl.setAttribute("open","")}catch(error){console.error("No se pudo abrir Nuevo jugador",error);playerDialogEl.setAttribute("open","")}
}
headerAddPlayerEl?.addEventListener("click",openNewPlayerForm);
playersAddPlayerEl?.addEventListener("click",openNewPlayerForm);

window.editPlayer=id=>openPlayerRecord(id);
  ...
  if(trashPlayers){
    ...
    trashPlayers.innerHTML=orderedDeletedPlayers.length?orderedDeletedPlayers.map(p=>`
      <article class="trash-item">
        ...
        <div class="trash-actions"><button class="primary small" onclick="restorePlayer('${p.id}')">Restaurar</button><button class="mini danger-mini" onclick="permanentlyDeletePlayer('${p.id}')">Eliminar definitivamente</button></div>
  ...
  if(trashTeams){
    ...
    trashTeams.innerHTML=orderedDeletedTeams.length?orderedDeletedTeams.map(t=>`
      <article class="trash-item">
        ...
        <div class="trash-actions"><button class="primary small" onclick="restoreTeam('${t.id}')">Restaurar</button><button class="mini danger-mini" onclick="permanentlyDeleteTeam('${t.id}')">Eliminar definitivamente</button></div>
...
function permanentDeleteConfirm(label){
  if(!confirm(`¿ELIMINAR DEFINITIVAMENTE ${label}?\n\nEsta acción es irreversible y no se podrá recuperar desde la papelera.`))return false;
  return confirm(`Última confirmación.\n\n¿Seguro que quieres borrar definitivamente ${label}?`);
}

async function deleteRowsByPlayerId(table,playerId,{optional=false}={}){
  const {error}=await sb.from(table).delete().eq("player_id",playerId);
  if(error){
    if(optional){console.warn(`Limpieza opcional ${table}`,error);return;}
    throw new Error(`${table}: ${supabaseErrorText(error)}`);
  }
}

async function removePlayerStorageFiles(player,documentRow){
  const paths=[
    player?.photo_path,
    documentRow?.player_dni_path,
    documentRow?.guardian_dni_path,
    documentRow?.photo_path,
    documentRow?.medical_path
  ].filter(Boolean);
  const unique=[...new Set(paths.map(path=>multiclubStoragePrefix(path)))];
  if(!unique.length)return;
  try{
    const {error}=await sb.storage.from("documents").remove(unique);
    if(error)console.warn("No se pudieron eliminar todos los archivos físicos del jugador",error);
  }catch(error){console.warn("Archivos físicos del jugador",error)}
}

window.permanentlyDeletePlayer=async id=>{
  const p=deletedPlayers.find(x=>String(x.id)===String(id));
  if(!p)return toast("Jugador no encontrado en la papelera");
  const label=`al jugador ${`${p.name||""} ${p.surname||""}`.trim()||"seleccionado"}`;
  if(!permanentDeleteConfirm(label))return;
  try{
    const documentRow=documents.find(row=>String(row.player_id)===String(id))||null;
    // Primero se eliminan las tablas dependientes para no dejar referencias huérfanas.
    for(const table of ["payments","documents","player_sizes","kits"]){
      await deleteRowsByPlayerId(table,id);
    }
    for(const table of ["player_custom_clothing_sizes","sports_training_attendance","sports_match_player_stats","team_player_cards","season_team_player_cards"]){
      await deleteRowsByPlayerId(table,id,{optional:true});
    }
    const {error}=await sb.from("players").delete().eq("id",id);
    if(error)throw new Error(`players: ${supabaseErrorText(error)}`);
    await removePlayerStorageFiles(p,documentRow);
    if(customPlayerClothingSizes&&Object.prototype.hasOwnProperty.call(customPlayerClothingSizes,id)){
      delete customPlayerClothingSizes[id];
      try{await saveCustomClothingSizes()}catch(error){console.warn("Tallaje personalizado",error)}
    }
    await logActivity("eliminar_definitivo","jugador",id,`Jugador eliminado definitivamente: ${p.name||""} ${p.surname||""}`.trim());
    toast("Jugador eliminado definitivamente");
    await loadAll();
  }catch(error){showOperationError("No se pudo eliminar definitivamente el jugador.",error)}
};

async function deleteRowsByTeamId(table,teamId,{optional=false}={}){
...
window.permanentlyDeleteTeam=async id=>{
  const t=deletedTeams.find(x=>String(x.id)===String(id));
  if(!t)return toast("Equipo no encontrado en la papelera");
  try{
    const {data:linkedPlayers,error:linkedError}=await sb.from("players").select("id,name,surname,deleted_at").eq("team_id",id).limit(10);
    if(linkedError)console.warn("Comprobación de jugadores vinculados",linkedError);
    if((linkedPlayers||[]).length){
      const names=linkedPlayers.slice(0,3).map(p=>`${p.name||""} ${p.surname||""}`.trim()).filter(Boolean).join(", ");
      alert(`No se puede eliminar definitivamente el equipo porque todavía tiene jugadores vinculados${names?`: ${names}`:""}.\n\nCambia esos jugadores de equipo o elimínalos definitivamente primero.`);
      return;
    }
    if(!permanentDeleteConfirm(`el equipo "${t.name||"seleccionado"}"`))return;

    // Estadísticas de partidos antes de eliminar los propios partidos.
    const matchQuery=await sb.from("sports_matches").select("id").eq("team_id",id);
    if(!matchQuery.error&&matchQuery.data?.length){
      const matchIds=matchQuery.data.map(row=>row.id).filter(Boolean);
      if(matchIds.length){
        const statsDelete=await sb.from("sports_match_player_stats").delete().in("match_id",matchIds);
        if(statsDelete.error)console.warn("Estadísticas vinculadas al equipo",statsDelete.error);
      }
    }

    for(const table of ["technical_user_teams","club_calendar_matches","team_player_cards","season_team_player_cards","sports_training_attendance"]){
      await deleteRowsByTeamId(table,id,{optional:true});
    }
    for(const table of ["sports_training_sessions","sports_matches"]){
      await deleteRowsByTeamId(table,id,{optional:true});
    }
    // Un usuario técnico puede conservarse, pero deja de apuntar al equipo eliminado.
    try{
      const clearTechnical=await sb.from("technical_users").update({team_id:null}).eq("team_id",id);
      if(clearTechnical.error)console.warn("Usuario técnico vinculado",clearTechnical.error);
    }catch(error){console.warn("Usuario técnico vinculado",error)}

    const {error}=await sb.from("teams").delete().eq("id",id);
    if(error)throw new Error(`teams: ${supabaseErrorText(error)}`);
    await logActivity("eliminar_definitivo","equipo",id,`Equipo eliminado definitivamente: ${t.name||""}`);
    toast("Equipo eliminado definitivamente");
    await loadAll();
  }catch(error){showOperationError("No se pudo eliminar definitivamente el equipo.",error)}
};
function activityIcon(action){
  return ({crear:"＋",actualizar:"✎",eliminar:"♻",restaurar:"↺",eliminar_definitivo:"✕",copia:"⬇",anular:"⊘",reactivar:"↻",resultado:"⚽",temporada:"▦"})[action]||"•";
}
...
const REALTIME_TABLES_V1036=[
  "club_seasons","players","payments","payment_concepts","documents","kits","teams","staff","events",
  "player_sizes","staff_sizes","pitch_usage","finance_movements","sponsorships","sponsorship_payments",
  "club_equipment_state","activity_logs","sports_training_sessions","sports_training_attendance",
  "sports_matches","sports_match_player_stats","club_calendar_matches","team_player_cards"
];
const SPORTS_TABLES_V1036=new Set([
  "sports_training_sessions","sports_training_attendance","sports_matches","sports_match_player_stats"
]);
const DIRECT_TABLES_V1036=new Set([
  "players","payments","payment_concepts","documents","kits","teams","staff","events",
  "player_sizes","staff_sizes","pitch_usage","finance_movements","sponsorships","sponsorship_payments","activity_logs"
]);
let realtimePendingV1036=new Map();
let realtimeTimerV1036=null;
let realtimeFlushPromiseV1036=null;
let realtimeNeedsAnotherFlushV1036=false;
let realtimeSignalsV1036=0;
let realtimeBatchesV1036=0;
let realtimePayloadEventsV1036=0;
let realtimeFallbackReadsV1036=0;
let realtimeFullReloadsV1036=0;
let calendarDirtyV1036=false;

function sportsUiVisibleV1036(){
  const activeView=document.querySelector("#app .view.active");
  if(activeView?.id==="sports")return true;
  const dialog=document.getElementById("teamDetailDialog");
  return !!(dialog?.open&&String(window.currentTeamDetailTab||"").startsWith("sports"));
}
function calendarUiVisibleV1036(){
  const activeView=document.querySelector("#app .view.active");
  if(activeView?.id==="calendar")return true;
  const dialog=document.getElementById("teamDetailDialog");
  return !!(dialog?.open&&["calendar","sportsMatches"].includes(String(window.currentTeamDetailTab||"")));
}
function currentRowsV1036(table){
  if(table==="players")return players;
  if(table==="payments")return payments;
  if(table==="payment_concepts")return paymentConcepts;
  ...
  if(table==="player_sizes")return playerSizes;
  if(table==="staff_sizes")return staffSizes;
  if(table==="pitch_usage")return pitchUsage;
  if(table==="finance_movements")return financeMovements;
  if(table==="sponsorships")return sponsorships;
  if(table==="sponsorship_payments")return sponsorshipPayments;
  if(table==="activity_logs")return activityLogs;
  return null;
}
function assignRowsV1036(table,rows){
  const data=Array.isArray(rows)?rows:[];
  if(table==="players")players=data;
  else if(table==="payments")payments=data;
  else if(table==="payment_concepts")paymentConcepts=data;
  else if(table==="documents")documents=data;
  else if(table==="kits")kits=data;
  else if(table==="teams")teams=data;
  else if(table==="staff")staff=data;
  else if(table==="events")events=data;
  else if(table==="player_sizes")playerSizes=data;
  else if(table==="staff_sizes")staffSizes=data;
  else if(table==="pitch_usage")pitchUsage=data;
  else if(table==="finance_movements")financeMovements=data;
  else if(table==="sponsorships")sponsorships=data;
  else if(table==="sponsorship_payments")sponsorshipPayments=data;
  else if(table==="activity_logs")activityLogs=data;
}
function rowAllowedV1036(table,row){
  if(table==="players")return !row?.deleted_at;
  if(table==="teams"){
    if(row?.deleted_at)return false;
    const sid=String(activeClubSeason?.id||"");
    return !sid||!row?.season_id||String(row.season_id)===sid;
  }
  return true;
}
function sortRowsV1036(table,rows){
  const data=[...rows];
  if(table==="teams"||table==="staff")data.sort((a,b)=>String(a?.name||"").localeCompare(String(b?.name||""),"es",{sensitivity:"base"}));
  else if(table==="events")data.sort((a,b)=>String(a?.event_date||"").localeCompare(String(b?.event_date||"")));
  else if(table==="pitch_usage")data.sort((a,b)=>String(b?.usage_date||"").localeCompare(String(a?.usage_date||"")));
  else if(table==="finance_movements")data.sort((a,b)=>String(b?.movement_date||"").localeCompare(String(a?.movement_date||"")));
  else if(table==="sponsorships")data.sort((a,b)=>String(b?.created_at||"").localeCompare(String(a?.created_at||"")));
  else if(table==="activity_logs")data.sort((a,b)=>String(b?.created_at||"").localeCompare(String(a?.created_at||"")));
  return data;
}
function applyPayloadsV1036(table,payloads){
  const source=currentRowsV1036(table);
  if(!Array.isArray(source)||!Array.isArray(payloads)||!payloads.length)return false;
  let rows=[...source],applied=0;
  for(const payload of payloads){
    const event=String(payload?.eventType||payload?.event_type||"").toUpperCase();
    if(event==="DELETE"){
    ...
    if(event!=="INSERT"&&event!=="UPDATE")return false;
    const row=payload?.new;
    const id=row?.id;
    if(id===undefined||id===null||id==="")return false;
    const index=rows.findIndex(item=>String(item?.id)===String(id));
    if(!rowAllowedV1036(table,row)){
      if(index>=0)rows.splice(index,1);
      if(table==="players"||table==="teams")trashLoadedV1035=false;
      applied++;
      continue;
    }
    ...
    if(table==="players"&&previous?.photo_path&&String(previous.photo_path)!==String(merged.photo_path||"")){
      delete signedCache[previous.photo_path];
    }
    if(table==="sponsorships"&&previous?.logo_path&&String(previous.logo_path)!==String(merged.logo_path||"")){
      delete sponsorshipLogoUrls[id];
    }
    applied++;
  }
  rows=sortRowsV1036(table,rows);
  if(table==="activity_logs")rows=rows.slice(0,150);
  if(table==="events"&&!eventsFullLoadedV1040){
    const today=todayIsoV1040();
    rows=rows.filter(row=>String(row?.event_date||"")>=today)
      .sort((a,b)=>String(a?.event_date||"").localeCompare(String(b?.event_date||"")))
      .slice(0,EVENTS_DASHBOARD_LIMIT_V1040);
  }
  assignRowsV1036(table,rows);
  realtimePayloadEventsV1036+=applied;
  return true;
}
async function refreshTableV1036(table){
  let q;
  if(table==="players")q=sb.from("players").select("id,name,surname,birth_date,player_dni,team,guardian,phone,email,address,notes,status,photo_path,created_at,deleted_at").is("deleted_at",null).order("created_at",{ascending:false});
  else if(table==="payments")q=sb.from("payments").select("id,player_id,concept,status,amount");
  else if(table==="payment_concepts")q=sb.from("payment_concepts").select("*").order("sort_order").order("name");
  else if(table==="documents")q=sb.from("documents").select("id,player_id,player_dni_status,guardian_dni_status,photo_status,medical_status,player_dni_path,guardian_dni_path,photo_path,medical_path");
  else if(table==="kits")q=sb.from("kits").select("id,player_id,sized,delivered,delivery_date");
  else if(table==="teams")q=sb.from("teams").select("id,name,age_category,category,coach_id,delegate_name,training_schedule,field,notes,season_id,created_at,deleted_at").eq("season_id",activeClubSeason.id).is("deleted_at",null).order("name");
  else if(table==="staff")q=sb.from("staff").select("id,name,role,team_name,phone,email,notes,photo_path,created_at").order("name");
  else if(table==="events")q=eventsFullLoadedV1040
    ? sb.from("events").select(EVENT_COLUMNS_V1040).order("event_date")
    : dashboardEventsQueryV1040();
  else if(table==="player_sizes")q=sb.from("player_sizes").select("*");
  else if(table==="staff_sizes")q=sb.from("staff_sizes").select("*");
  else if(table==="pitch_usage")q=sb.from("pitch_usage").select("*").order("usage_date",{ascending:false});
  else if(table==="finance_movements")q=sb.from("finance_movements").select("*").order("movement_date",{ascending:false});
  else if(table==="sponsorships")q=sb.from("sponsorships").select("*").order("created_at",{ascending:false});
  else if(table==="sponsorship_payments")q=sb.from("sponsorship_payments").select("*").order("payment_date",{ascending:false}).order("created_at",{ascending:false});
  else if(table==="activity_logs")q=sb.from("activity_logs").select(ACTIVITY_COLUMNS_V1035).eq("club_id",activeClubId()).order("created_at",{ascending:false}).limit(document.getElementById("activity")?.classList.contains("active")?150:6);
  else return false;
  const {data,error}=await q;
  if(error)throw error;
  assignRowsV1036(table,sortRowsV1036(table,data||[]));
  realtimeFallbackReadsV1036++;
  return true;
}
async function flushRealtimeV1036(){
  if(realtimeFlushPromiseV1036){realtimeNeedsAnotherFlushV1036=true;return realtimeFlushPromiseV1036}
  if(!realtimePendingV1036.size||isHistoricalSeason())return false;
  const entries=new Map(realtimePendingV1036);
  realtimePendingV1036.clear();
  realtimeBatchesV1036++;
  realtimeFlushPromiseV1036=(async()=>{
    if(entries.has("club_seasons")){
    ...
    let repaint=false;
    for(const [table,payloads] of entries){
      if(table==="club_seasons")continue;
      if(DIRECT_TABLES_V1036.has(table)){
        if(!applyPayloadsV1036(table,payloads))await refreshTableV1036(table);
        repaint=true;
        continue;
      }
      if(SPORTS_TABLES_V1036.has(table)){
        sportsLoadedV1032=false;
        if(sportsUiVisibleV1036())await ensureSportsLoadedV1032();
        continue;
      }
      if(table==="club_calendar_matches"){
        calendarDirtyV1036=true;
        if(calendarUiVisibleV1036()&&typeof window.hydrateClubMatchesFromCloudV27244==="function"){
          await window.hydrateClubMatchesFromCloudV27244({migrateLocal:false});
          calendarDirtyV1036=false;
        }
        continue;
      }
      if(table==="club_equipment_state"){
        if(document.getElementById("kits")?.classList.contains("active"))await loadMulticlubEquipmentState();
        continue;
      }
      if(table==="team_player_cards"){
        if(document.getElementById("teamDetailDialog")?.open&&String(window.currentTeamDetailTab||"")==="cards"){
          window.renderTeamDetailTab?.("cards");
        }
      }
    }
    if(repaint)render();
    ...
    console.warn("Multiclub V1.0.36 · Realtime directo",error);
    realtimeFullReloadsV1036++;
    await loadAll();
    return false;
  }).finally(()=>{
    realtimeFlushPromiseV1036=null;
    if(realtimeNeedsAnotherFlushV1036||realtimePendingV1036.size){
      realtimeNeedsAnotherFlushV1036=false;
      scheduleRealtimeFlushV1036();
    }
  });
  return realtimeFlushPromiseV1036;
}
function scheduleRealtimeFlushV1036(){
  if(realtimeTimerV1036)return;
  realtimeTimerV1036=setTimeout(()=>{
    realtimeTimerV1036=null;
    flushRealtimeV1036().catch(error=>console.warn("Multiclub V1.0.36 · Realtime",error));
  },40);
}
function noteRealtimeV1036(table,payload){
  if(isHistoricalSeason())return;
  realtimeSignalsV1036++;
  const key=String(table||"");
  const list=realtimePendingV1036.get(key)||[];
  list.push(payload||{});
  realtimePendingV1036.set(key,list);
  scheduleRealtimeFlushV1036();
}
document.addEventListener("click",event=>{
  const nav=event.target.closest?.(".nav[data-view]");
  if(!nav||!calendarDirtyV1036)return;
  if(String(nav.dataset.view||"")==="calendar"&&typeof window.hydrateClubMatchesFromCloudV27244==="function"){
    window.hydrateClubMatchesFromCloudV27244({migrateLocal:false}).then(()=>{calendarDirtyV1036=false}).catch(error=>console.warn("Calendario realtime",error));
  }
},true);
function realtime(){
  ...
  let next=sb.channel(`club-live:${clubId}`);
  for(const table of REALTIME_TABLES_V1036){
    next=next.on("postgres_changes",{
      event:"*",schema:"public",table,filter:`club_id=eq.${clubId}`
    },payload=>noteRealtimeV1036(table,payload));
  }
  channel=next.subscribe();
  window.MULTICLUB_REALTIME_V1036=Object.freeze({
    active:true,
    clubId,
    metrics:()=>({
      signals:realtimeSignalsV1036,
      batches:realtimeBatchesV1036,
      payloadEvents:realtimePayloadEventsV1036,
      fallbackReads:realtimeFallbackReadsV1036,
      fullReloads:realtimeFullReloadsV1036,
      pending:[...realtimePendingV1036.keys()],
      calendarDirty:calendarDirtyV1036,
      refreshing:!!realtimeFlushPromiseV1036
    }),
    refresh:()=>{realtimePendingV1036.set("manual",[{eventType:"MANUAL"}]);return flushRealtimeV1036()}
  });
  return channel;
}
...
window.deleteStaff=async id=>{
  ...
  if(!member)return toast("Miembro no encontrado");
  if(!confirm(`¿Eliminar a ${member.name||"este miembro"} del cuerpo técnico?\n\nSi figura como entrenador de algún equipo, quedará desasignado automáticamente.`))return;
  try{
    // V1.0.31: desasignar primero al técnico de TODOS los equipos del club.
    // Esto evita que una FK antigua intente poner teams.club_id a NULL al borrar staff.
    const unlink=await sb.from("teams").update({coach_id:null}).eq("coach_id",id);
    if(unlink.error)throw new Error("No se pudo desasignar al técnico de los equipos: "+(unlink.error.message||"error desconocido"));

    const removed=await sb.from("staff").delete().eq("id",id);
    if(removed.error)throw removed.error;

    delete staffKitStatus[id];
    saveStaffKitStatus();
    delete customStaffClothingSizes[id];
    saveCustomClothingSizes();
    await logActivity("eliminar","cuerpo_tecnico",id,`Miembro eliminado: ${member.name||"Cuerpo técnico"}`);
    toast("Miembro eliminado");
    await loadAll();
  }catch(error){
    showOperationError("No se pudo eliminar el miembro.",error);
  }
};
...
window.renderTeamDetailTab=function(tab){
  ...
  if(String(tab||"").startsWith("sports")){
    ensureSportsLoadedV1032().catch(error=>console.warn("Multiclub V1.0.32 · Deportes",error));
  }
  ...
  const coachMember=staff.find(s=>String(s.id)===String(t.coach_id));
  const cardStaffList=[...staffList];
  if(coachMember&&!cardStaffList.some(s=>String(s.id)===String(coachMember.id)))cardStaffList.unshift(coachMember);
  const coach=coachMember?.name||staffList.find(s=>s.role==="Entrenador")?.name||"Sin asignar";
...
function renderDashboard(){
  ...
  recent.innerHTML=players.slice(0,6).map(p=>`
    <button class="dashboard-player-row" onclick="editPlayer('${p.id}')">
      <img src="${dashboardPlayerPhoto(p)}" data-photo-path="${esc(p.photo_path||"")}" alt="Fotografía de ${esc(p.name||"jugador")}" onerror="this.src='${placeholderPhoto}'">
...
window.getMulticlubCalendarMatchesV1034=()=>Array.isArray(clubMatches)?clubMatches.map(item=>({...item})):[];
...
function weeklyMatchesV1028(){
  const now=new Date(),day=(now.getDay()+6)%7,start=new Date(now);start.setHours(0,0,0,0);start.setDate(now.getDate()-day);const end=new Date(start);end.setDate(start.getDate()+7);
  return (clubMatches||[]).filter(m=>{if(!m?.match_date)return false;const d=new Date(m.match_date+"T12:00:00");return d>=start&&d<end}).sort((a,b)=>`${a.match_date} ${a.match_time||""}`.localeCompare(`${b.match_date} ${b.match_time||""}`));
}
function downloadWeeklyMatchesPdfV1028(){
  try{
    const matches=weeklyMatchesV1028();
    if(!matches.length){toast("No hay partidos esta semana para generar el PDF");return;}
    const JsPdf=window.jspdf?.jsPDF;if(!JsPdf)throw new Error("No está disponible el generador de PDF");
    const doc=new JsPdf({orientation:"landscape",unit:"mm",format:"a4"});
    const now=new Date(),day=(now.getDay()+6)%7,start=new Date(now);start.setHours(0,0,0,0);start.setDate(now.getDate()-day);const finish=new Date(start);finish.setDate(start.getDate()+6);
    doc.setFont("helvetica","bold");doc.setFontSize(18);doc.text(`${currentClubFullName()} · Partidos de la semana`,14,16);
    doc.setFont("helvetica","normal");doc.setFontSize(10);doc.text(`${start.toLocaleDateString("es-ES")} - ${finish.toLocaleDateString("es-ES")} · Temporada ${currentSeasonLabel()}`,14,23);
    const rows=matches.map(m=>{
      const home=m.venue!=="away";
      const result=matchIsPlayed(m)?`${m.goals_for ?? ""}-${m.goals_against ?? ""}`:"Pendiente";
      return [reportDate(m.match_date),m.match_time||"—",m.team||"—",m.opponent||"—",home?"Local":"Visitante",m.competition||"—",m.round||"—",m.location||"—",result];
    });
    if(typeof doc.autoTable==="function"){
      doc.autoTable({startY:29,head:[["Fecha","Hora","Equipo","Rival","Condición","Competición","Jornada","Campo","Resultado"]],body:rows,styles:{fontSize:8,cellPadding:2.2},headStyles:{fontStyle:"bold"},columnStyles:{0:{cellWidth:22},1:{cellWidth:16},4:{cellWidth:20},8:{cellWidth:20}}});
    }else{
      let y=31;doc.setFontSize(8);
      for(const row of rows){doc.text(row.join("   |   ").slice(0,180),14,y);y+=5;if(y>195){doc.addPage();y=15}}
    }
    const safe=String(currentClubShortName()||"club").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"");
    doc.save(`${safe||"club"}_partidos_semana_${start.toISOString().slice(0,10)}.pdf`);
    toast(`PDF semanal generado · ${matches.length} partido(s)`);
  }catch(error){console.error("PDF semanal",error);showOperationError("No se pudo generar el PDF semanal.",error)}
}
window.downloadWeeklyMatchesPdfV1028=downloadWeeklyMatchesPdfV1028;

function setupMatchesModule(){
  loadClubMatches();renderClubMatches();renderDashboardMatches();
  document.getElementById("weeklyMatchesPdf")?.addEventListener("click",downloadWeeklyMatchesPdfV1028);
...
const RIVAL_TEAMS_LEGACY_KEY_V1039="cdsb_rival_teams_v24_5_4";
const RIVAL_TEAMS_OWNER_KEY_V1039="manager_multiclub_rivals_legacy_owner_v1039";
let rivalTeams=[];
function rivalTeamsStorageKeyV1039(){
  let club="";
  try{club=String(activeClubId?.()||currentMulticlubClub?.id||"").trim()}catch(_){club=String(currentMulticlubClub?.id||"").trim()}
  return `manager_multiclub_rival_teams_v1039_${club||"sin_club"}`;
}
function migrateLegacyRivalTeamsV1039(){
  try{
    const club=String(activeClubId?.()||currentMulticlubClub?.id||"").trim();
    if(!club)return;
    const key=rivalTeamsStorageKeyV1039();
    if(localStorage.getItem(key)!=null)return;
    const owner=String(localStorage.getItem(RIVAL_TEAMS_OWNER_KEY_V1039)||"").trim();
    if(owner&&owner!==club)return;
    const legacy=localStorage.getItem(RIVAL_TEAMS_LEGACY_KEY_V1039);
    if(!legacy)return;
    const parsed=JSON.parse(legacy);
    if(!Array.isArray(parsed)||!parsed.length)return;
    localStorage.setItem(key,JSON.stringify(parsed));
    localStorage.setItem(RIVAL_TEAMS_OWNER_KEY_V1039,club);
    localStorage.removeItem(RIVAL_TEAMS_LEGACY_KEY_V1039);
  }catch(error){console.warn("Migración de equipos rivales",error)}
}
function loadRivalTeams(){
  try{
    migrateLegacyRivalTeamsV1039();
    const raw=JSON.parse(localStorage.getItem(rivalTeamsStorageKeyV1039())||"[]");
    rivalTeams=Array.isArray(raw)?raw:[];
  }catch(_){rivalTeams=[]}
}
function saveRivalTeams(options={}){
  try{
    localStorage.setItem(rivalTeamsStorageKeyV1039(),JSON.stringify(rivalTeams));
    return true;
  }catch(error){
