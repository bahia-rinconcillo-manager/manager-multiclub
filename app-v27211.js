...
const CLOTHING_SIZE_ORDER=["4XS","3XS","2XS","XS","S","M","L","XL","2XL","3XL"];
...
function clothingItemOrderForLabel(label){
  const itemIndex=CLOTHING_ITEMS.findIndex(item=>clothingConfig(item).label===label);
...
window.renderTeamDetailTab=function(tab){
  ...
  teamDetailContent.innerHTML=`<nav class="team-detail-tabs">${tabs.map(([id,label])=>`<button class="${tab===id?"active":""} ${id.startsWith("sports")?"sports-tab":""} ${id==="cards"?"cards-tab":""}" onclick="renderTeamDetailTab('${id}')">${id==="cards"?"▣ ":id==="accidents"?"✚ ":id==="sportsAttendance"?"✓ ":id==="sportsMatches"?"⚽ ":id==="sportsStats"?"▥ ":""}${label}</button>`).join("")}</nav><div class="team-detail-tab-content">${body}</div>`;
  if(tab==="cards"){
