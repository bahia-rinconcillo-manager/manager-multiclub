-- ============================================================================
-- CD SAN BERNABÉ MANAGER · V27.2.6
-- CATEGORÍA DE EDAD + MODALIDAD EN EQUIPOS POR TEMPORADA
-- Ejecutar DESPUÉS de V27.2.5
-- ============================================================================
--
-- Separa dos conceptos que antes estaban mezclados:
--   · Categoría de edad: Bebé, Prebenjamín, Benjamín, Alevín, Infantil, Cadete, Juvenil.
--   · Modalidad: Fútbol 7, Fútbol 11 u otra denominación operativa.
-- La asignación automática del cambio anual utiliza exclusivamente la categoría de edad.
-- ============================================================================

begin;

-- 1. Nueva columna estable para la categoría de edad.
alter table public.teams
  add column if not exists age_category text;

-- Completar automáticamente los equipos actuales a partir de su nombre.
update public.teams
set age_category = case
  when lower(name) like '%prebenj%' then 'Prebenjamín'
  when lower(name) like '%benj%' then 'Benjamín'
  when lower(name) like '%alev%' then 'Alevín'
  when lower(name) like '%infantil%' then 'Infantil'
  when lower(name) like '%cadete%' then 'Cadete'
  when lower(name) like '%juvenil%' then 'Juvenil'
  when lower(name) like '%bebe%' or lower(name) like '%bebé%' then 'Bebé'
  else age_category
end
where age_category is null or trim(age_category) = '';

-- 2. Cambio anual V27.2.6.
create or replace function public.cdsb_rollover_season_v2726(
...
begin
  ...
  insert into public.teams(
    name, age_category, category, coach_id, delegate_name,
    training_schedule, field, notes, season_id
  )
  select
    trim(plan.item->>'name'),
    nullif(trim(plan.item->>'age_category'), ''),
...
revoke all on function public.cdsb_rollover_season_v2726(uuid,text,jsonb,jsonb,jsonb,jsonb,boolean,boolean,text) from public;
grant execute on function public.cdsb_rollover_season_v2726(uuid,text,jsonb,jsonb,jsonb,jsonb,boolean,boolean,text) to authenticated;
...
select
  'V27.2.6 instalada correctamente: categoría de edad separada de la modalidad' as resultado,
  ...
  (select count(*) from public.teams t where t.season_id = s.id and t.deleted_at is null and t.age_category is not null) as equipos_con_categoria_edad
