// CD San Bernabé Manager · V27.2.20 · impresión/PDF de tallaje técnico corregida
...
function printStaffSizingReport(){
  const rows=staffSizingRows();
  if(!rows.length)return toast("No hay tallajes del cuerpo técnico para imprimir");
  const labels=Object.keys(rows[0]);
  const team=document.getElementById("staffSizingTeam")?.value||"Todos los equipos";

  // V27.2.20: imprimir desde la propia aplicación. Evita window.open(), que
  // algunos navegadores y las PWA instaladas bloquean aunque el usuario pulse
  // directamente el botón Imprimir / PDF.
  document.getElementById("staffSizingPrintRoot")?.remove();
  document.getElementById("staffSizingPrintStyle")?.remove();

  const style=document.createElement("style");
  style.id="staffSizingPrintStyle";
  style.textContent=`
    #staffSizingPrintRoot{display:none}
    @media print{
      @page{size:landscape;margin:10mm}
      body.staff-sizing-print-mode{background:#fff!important;overflow:visible!important}
      body.staff-sizing-print-mode > *:not(#staffSizingPrintRoot){display:none!important}
      body.staff-sizing-print-mode #staffSizingPrintRoot{display:block!important;position:static!important;width:100%!important;margin:0!important;padding:0!important;color:#102a43!important;background:#fff!important}
      #staffSizingPrintRoot h1{font:700 20px Arial,sans-serif;color:#0b4da2;margin:0 0 3px}
      #staffSizingPrintRoot p{font:12px Arial,sans-serif;color:#52606d;margin:0 0 10px}
      #staffSizingPrintRoot .meta{font:700 12px Arial,sans-serif;margin:0 0 10px}
      #staffSizingPrintRoot table{width:100%;border-collapse:collapse;table-layout:auto;font:9px Arial,sans-serif}
      #staffSizingPrintRoot th,#staffSizingPrintRoot td{border:1px solid #cbd5e1;padding:4px;text-align:left;vertical-align:top;word-break:normal;overflow-wrap:anywhere}
      #staffSizingPrintRoot th{background:#0b4da2!important;color:#fff!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}
      #staffSizingPrintRoot tr{break-inside:avoid;page-break-inside:avoid}
    }`;
  document.head.appendChild(style);

  const root=document.createElement("section");
  root.id="staffSizingPrintRoot";
  root.innerHTML=`<h1>CD SAN BERNABÉ</h1><p>Tallaje exclusivo del cuerpo técnico</p><div class="meta">Equipo: ${esc(team)} · ${rows.length} miembro${rows.length===1?"":"s"}</div><table><thead><tr>${labels.map(x=>`<th>${esc(x)}</th>`).join("")}</tr></thead><tbody>${rows.map(row=>`<tr>${labels.map(k=>`<td>${esc(row[k]??"-")}</td>`).join("")}</tr>`).join("")}</tbody></table>`;
  document.body.appendChild(root);
  document.body.classList.add("staff-sizing-print-mode");

  let cleaned=false;
  const cleanup=()=>{
    if(cleaned)return;
    cleaned=true;
    document.body.classList.remove("staff-sizing-print-mode");
    root.remove();
    style.remove();
  };
  ...
  // Deja que el navegador componga el documento antes de abrir el diálogo.
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    try{
      window.print();
      // Safari/iOS no siempre lanza afterprint al cancelar.
      setTimeout(cleanup,3000);
    }catch(error){
      console.error("No se pudo abrir la impresión del tallaje técnico",error);
      cleanup();
      toast("No se pudo abrir Imprimir / PDF en este dispositivo");
    }
  }));
}
...
document.getElementById("printStaffSizingReport")?.addEventListener("click",printStaffSizingReport);
...
async function start(){
  ...
  if(isTechnicalReadOnly())await ensureTechnicalTeamChoice(false);
  await loadClubSeasons();
...
async function loadTeamUserProfiles(){
  ...
  const mappingResult=await sb.rpc("admin_list_technical_user_teams_v27219");
  if(!mappingResult.error&&Array.isArray(mappingResult.data)){
    ...
    if(usersSetupWarning){
      ...
      usersSetupWarning.textContent="Para asignar varios equipos al mismo entrenador ejecuta EJECUTAR_ACTUALIZACION_V27_2_19.sql.";
    }
  }
  renderTeamUsers();
}
...
async function technicalAvailableTeams(){
  if(!isTechnicalReadOnly())return [];
  const token=getTechnicalSessionToken();if(!token)return [];
  const {data,error}=await sb.rpc("technical_available_teams_v27219",{p_token:token});
  if(error){
    console.warn("Accesos multiequipo no disponibles",error);
    return currentAccess?.team_id?[{team_id:currentAccess.team_id,team_name:"Equipo asignado",is_primary:true}]:[];
  }
...
async function setTechnicalActiveTeam(teamId){
  if(!teamId)return;
  const token=getTechnicalSessionToken();
  const {data,error}=await sb.rpc("technical_select_team_v27219",{p_token:token,p_team_id:teamId});
  if(error)throw error;
...
async function ensureTechnicalTeamChoice(force=false){
  ...
  const remembered=force?"":localStorage.getItem(TECH_TEAM_CHOICE_KEY);
  if(remembered&&currentTechnicalTeams.some(team=>String(team.team_id)===String(remembered))){
    await setTechnicalActiveTeam(remembered);
    return;
  }
...
switchTechnicalTeam?.addEventListener("click",async()=>{
  if(!isTechnicalReadOnly())return;
  try{
    await ensureTechnicalTeamChoice(true);
    if(typeof loadClubMatches==="function")loadClubMatches();
    await loadAll();
    await window.sportsV2700?.load?.();
    await applyCurrentAccess();
    toast("Equipo cambiado correctamente");
  }catch(error){toast("No se pudo cambiar de equipo: "+(error.message||error))}
});
