// CD San Bernabé Manager · V27.2.24 · multiequipo reforzado + reparación de asignaciones
...
async function loadTeamUserProfiles(){
  ...
  let mappingResult=await sb.rpc("admin_list_technical_user_teams_v27224");
  ...
  if(!mappingResult.error&&Array.isArray(mappingResult.data)){
    ...
    if(usersSetupWarning){
      ...
      usersSetupWarning.textContent="Para detectar automáticamente todos los equipos de cada entrenador ejecuta EJECUTAR_ACTUALIZACION_V27_2_24.sql.";
    }
  }
  renderTeamUsers();
}
...
async function technicalAvailableTeams(){
  ...
  // V27.2.24: primero usa el mapa reforzado. Este incluye asignaciones explícitas,
  // equipo del registro de cuerpo técnico, coach_id del equipo y delegado por nombre.
  // Además, la migración repara el caso de accesos antiguos con dos equipos.
  let result=await sb.rpc("technical_available_teams_v27224",{p_token:token});
  if(result.error){
    result=await sb.rpc("technical_available_teams_v27223",{p_token:token});
  }
  if(result.error){
    result=await sb.rpc("technical_available_teams_v27219",{p_token:token});
  }
  if(result.error){
    console.warn("Accesos multiequipo no disponibles",result.error);
...
async function setTechnicalActiveTeam(teamId){
  if(!teamId)return;
  const token=getTechnicalSessionToken();
  let result=await sb.rpc("technical_select_team_v27224",{p_token:token,p_team_id:teamId});
  if(result.error){
    result=await sb.rpc("technical_select_team_v27223",{p_token:token,p_team_id:teamId});
  }
  if(result.error){
    result=await sb.rpc("technical_select_team_v27219",{p_token:token,p_team_id:teamId});
  }
