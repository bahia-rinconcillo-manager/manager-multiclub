// V1.0.30 · Optimización de Egress, fotografías y sincronización sobre V1.0.29
...
let sb, players=[], payments=[], paymentConcepts=[], documents=[], kits=[], teams=[], staff=[], events=[], playerSizes=[], staffSizes=[], pitchUsage=[], financeMovements=[], sponsorships=[], sponsorshipPayments=[], deletedPlayers=[], deletedTeams=[], activityLogs=[], channel;
let signedCache={}, signedUrlMeta={}, clothingImages={}, sponsorshipLogoUrls={};
let clothingImagesLoaded=false;
let lastFullLoadStartedAt=0,lastFullLoadCompletedAt=0;
const SIGNED_URL_CACHE_KEY="manager_multiclub_signed_url_cache_v1030";
const SIGNED_URL_TTL_MS=50*60*1000;
const SIGNED_URL_MAX_ENTRIES=800;
function loadSignedUrlCache(){
  try{
    const raw=JSON.parse(localStorage.getItem(SIGNED_URL_CACHE_KEY)||"{}");
    const now=Date.now();
    Object.entries(raw||{}).forEach(([path,entry])=>{
      if(entry&&entry.url&&Number(entry.expiresAt||0)>now){signedCache[path]=entry.url;signedUrlMeta[path]={expiresAt:Number(entry.expiresAt)}}
    });
  }catch(_){signedCache={};signedUrlMeta={}}
}
function persistSignedUrlCache(){
  try{
    const now=Date.now();
    const entries=Object.entries(signedUrlMeta)
      .filter(([path,meta])=>signedCache[path]&&Number(meta?.expiresAt||0)>now)
      .sort((a,b)=>Number(b[1]?.expiresAt||0)-Number(a[1]?.expiresAt||0))
      .slice(0,SIGNED_URL_MAX_ENTRIES);
    const out={};for(const [path,meta] of entries)out[path]={url:signedCache[path],expiresAt:Number(meta.expiresAt)};
    localStorage.setItem(SIGNED_URL_CACHE_KEY,JSON.stringify(out));
  }catch(_){}
}
function invalidateSignedUrl(path){
  if(!path)return;
  delete signedCache[path];delete signedUrlMeta[path];persistSignedUrlCache();
}
function restoreRuntimeSignedCache(){signedCache={};signedUrlMeta={};loadSignedUrlCache()}
loadSignedUrlCache();
...
const MULTICLUB_BUILD_VERSION="V1.0.30";
  ...
  try{
    ...
    body.innerHTML=`
      <section class="histg-card histg-summary">
        <img class="histg-photo" ${storageImageAttributes(player.photo_path,{fallback:"assets/avatar-jugador.svg"})} alt="Foto del jugador" onerror="this.onerror=null;this.src='assets/avatar-jugador.svg'">
...
async function optimizeProfilePhoto(file){
  ...
  try{
    ...
    const blob=await new Promise((resolve,reject)=>canvas.toBlob(b=>b?resolve(b):reject(new Error("No se pudo optimizar la fotografía")),"image/jpeg",.84));
    const base=cleanName(String(file.name||"foto").replace(/\.[^.]+$/,""))||"foto";
    return new File([blob],`${base}.jpg`,{type:"image/jpeg",lastModified:Date.now()});
  }catch(error){console.warn("No se pudo optimizar la fotografía; se subirá el original",error);return file}
}
async function uploadPrivate(file,folder){
  if(!file||!file.name)return null;
  const isProfilePhoto=/\/photo\/?$/i.test(String(folder||""));
  const uploadFile=isProfilePhoto?await optimizeProfilePhoto(file):file;
  const path=multiclubStoragePrefix(`${folder}/${Date.now()}_${cleanName(uploadFile.name)}`);
  const options={contentType:uploadFile.type||undefined,cacheControl:isProfilePhoto?"604800":"0"};
  const {error}=await sb.storage.from("documents").upload(path,uploadFile,options);if(error)throw error;return path
}
async function signedUrl(path,{force=false}={}){
  if(!path)return null;
  const now=Date.now();
  const cached=signedCache[path],meta=signedUrlMeta[path];
  if(!force&&cached&&(!meta||Number(meta.expiresAt||0)>now+15000))return cached;
  if(force)invalidateSignedUrl(path);
  const {data,error}=await sb.storage.from("documents").createSignedUrl(path,3600);if(error)throw error;
  const url=data?.signedUrl||null;
  if(url){signedCache[path]=url;signedUrlMeta[path]={expiresAt:Date.now()+SIGNED_URL_TTL_MS};persistSignedUrlCache()}
  return url
}
const placeholderPhoto="assets/avatar-jugador.svg";
function storageImageAttributes(path,{fallback=placeholderPhoto}={}){
  const cached=path?signedCache[path]:null;
  const src=cached||fallback;
  const pending=path&&!cached?` data-storage-photo-path="${esc(path)}"`:"";
  return `src="${esc(src)}"${pending} loading="lazy" decoding="async"`;
}
let storageImageObserver=null;
async function resolveStorageImageElement(img){
  if(!img||img.dataset.storagePhotoLoading==="1")return;
  const path=img.dataset.storagePhotoPath;if(!path)return;
  img.dataset.storagePhotoLoading="1";
  try{
    const url=await signedUrl(path);
    if(url&&img.isConnected)img.src=url;
    delete img.dataset.storagePhotoPath;
  }catch(error){console.warn("No se pudo cargar una fotografía privada",error);if(img.isConnected)img.src=placeholderPhoto}
  finally{delete img.dataset.storagePhotoLoading}
}
function hydrateStorageImages(root=document){
  const images=[...(root?.querySelectorAll?.("img[data-storage-photo-path]")||[])];
  if(!images.length)return;
  if("IntersectionObserver" in window){
    if(!storageImageObserver)storageImageObserver=new IntersectionObserver(entries=>entries.forEach(entry=>{if(entry.isIntersecting){storageImageObserver.unobserve(entry.target);resolveStorageImageElement(entry.target)}}),{rootMargin:"350px 0px"});
    images.forEach(img=>storageImageObserver.observe(img));
  }else images.forEach(resolveStorageImageElement);
}
function initStorageImageHydration(){
  hydrateStorageImages(document);
  if(!document.body||document.body.dataset.storageImageObserver==="1")return;
  document.body.dataset.storageImageObserver="1";
  const observer=new MutationObserver(records=>{
    for(const record of records)for(const node of record.addedNodes||[]){
      if(node?.nodeType!==1)continue;
      if(node.matches?.("img[data-storage-photo-path]"))hydrateStorageImages(node.parentElement||document);
      else if(node.querySelector?.("img[data-storage-photo-path]"))hydrateStorageImages(node);
    }
  });
  observer.observe(document.body,{childList:true,subtree:true});
}
document.addEventListener("DOMContentLoaded",()=>setTimeout(initStorageImageHydration,0));
window.multiclubSignedUrl=signedUrl;
window.multiclubStorageImageAttributes=storageImageAttributes;
...
async function loadClothingImages(force=false){
  if(clothingImagesLoaded&&!force)return;
  ...
  try{
    ...
    const tasks=(data||[]).map(async item=>{
      ...
      const url=await signedUrl(path);
      const version=encodeURIComponent(String(item.updated_at||item.created_at||""));
      if(url)clothingImages[key]=version?`${url}${url.includes("?")?"&":"?"}v=${version}`:url;
    });
  ...
  grid.innerHTML=CLOTHING_CATEGORIES.map(category=>{
    ...
    return `<section class="clothing-category-block"><div class="clothing-category-title"><span>${esc(category)}</span><small>${items.length} prenda${items.length===1?"":"s"}</small></div><div class="clothing-category-grid">${items.map(item=>{
      ...
      return `<article class="clothing-item-card ${cfg.active?"":"is-inactive"}" data-clothing-item="${item.key}">
        ...
        <div class="clothing-item-image">${clothingImages[item.key]?`<img src="${clothingImages[item.key]}" alt="${esc(cfg.label)}" loading="lazy" decoding="async">`:`<span class="clothing-placeholder">${item.icon}</span>`}</div>
...
function refreshKitDialogImages(){
  document.querySelectorAll(".visual-size-field[data-clothing-key]").forEach(field=>{
    ...
    box.innerHTML=clothingImages[key]?`<img src="${clothingImages[key]}" alt="${esc(cfg?.label||"Prenda")}" loading="lazy" decoding="async">`:"";
  });
}
window.uploadClothingImage=async(key,input)=>{
  ...
  try{
    ...
    const {error}=await sb.storage.from("documents").upload(path,blob,{contentType:"image/jpeg",upsert:true,cacheControl:"604800"});
    if(error)throw error;
...
function applySnapshotTables(snapshot){
  ...
  restoreRuntimeSignedCache();clothingImages={};clothingImagesLoaded=false;
}
function hydrateHistoricalMediaInBackground(season){
  const seasonId=String(season?.id||"");
  // Las fotos privadas se resuelven bajo demanda al entrar en pantalla.
  // Solo se prepara el pequeño catálogo de ropa del club.
  Promise.resolve(loadClothingImages()).then(()=>{
...
async function returnToClubSelector(){
  ...
  try{
    ...
    deletedPlayers=[];deletedTeams=[];activityLogs=[];restoreRuntimeSignedCache();clothingImages={};clothingImagesLoaded=false;sponsorshipLogoUrls={};
...
async function loadTechnicalTeamData(){
  ...
  deletedTeams=[];
  activityLogs=[];
  restoreRuntimeSignedCache();
  clothingImages={};clothingImagesLoaded=false;
}
async function loadAll(){
  lastFullLoadStartedAt=Date.now();
  ...
  lastFullLoadCompletedAt=Date.now();
  render();
}
...
function render(){
  ...
  setTimeout(()=>hydrateStorageImages(document),0);
}
...
function renderPlayers(){
  ...
  grid.innerHTML=list.length?list.map(p=>{
    ...
    return `<article class="player-professional-card" data-open-player-record="${p.id}" data-season-readonly-allow="true">
      <div class="player-photo-wrap">
        <img ${storageImageAttributes(p.photo_path)} alt="Foto de ${esc(p.name||"jugador")}" onerror="this.onerror=null;this.src='${placeholderPhoto}'">
  ...
  tableBody.innerHTML=list.length?list.map(p=>{
    return `<tr>
      <td><img class="player-table-photo" ${storageImageAttributes(p.photo_path)} onerror="this.onerror=null;this.src='${placeholderPhoto}'"></td>
...
function renderCustomClothingFields(audience,values={}){
  ...
  container.innerHTML=items.length?`<div class="custom-clothing-fields-title">Prendas añadidas al catálogo</div>${items.map(item=>{
    ...
    return `<label class="visual-size-field custom-visual-size-field" data-clothing-key="${item.key}"><span class="visual-size-photo">${clothingImages[item.key]?`<img src="${clothingImages[item.key]}" alt="${esc(cfg.label)}" loading="lazy" decoding="async">`:""}</span><span class="visual-size-name">${esc(cfg.label)}</span><select name="custom__${item.key}">${options}</select></label>`;
  }).join("")}`:"";
}
...
function sponsorshipLogoMarkup(sponsor){
  ...
  if(url)return `<img class="sponsorship-list-logo" src="${esc(url)}" alt="Logo ${esc(sponsor.sponsor_name||"")}" loading="lazy" decoding="async">`;
...
function openHistoricalPlayerViewer(id){
  ...
  body.innerHTML=`
    <div class="historical-viewer-summary">
      <img ${storageImageAttributes(p.photo_path)} alt="Foto del jugador" onerror="this.onerror=null;this.src='${placeholderPhoto}'">
...
function openPlayerRecord(id){
  ...
  if(p.photo_path&&!signedCache[p.photo_path]){
    signedUrl(p.photo_path).then(url=>{
      if(url&&String(playerFormEl?.elements?.id?.value||"")===String(p.id))setPlayerProfilePreview(url);
    }).catch(error=>console.warn("No se pudo cargar la fotografía de la ficha",error));
  }
...
if(playerFormEl)playerFormEl.onsubmit=async e=>{
  ...
  try{
    ...
    const previousPhoto=(players.find(x=>String(x.id)===String(playerId))?.photo_path)||existing.photo_path||null;
    ...
    try{
      if(photoFile?.name){
        ...
        newPhotoPath=path;docData.photo_path=path;docData.photo_status="Recibido";
      ...
      if(newPhotoPath&&previousPhoto&&previousPhoto!==newPhotoPath){
        try{const rem=await sb.storage.from("documents").remove([previousPhoto]);if(rem.error)console.warn("No se pudo borrar la foto anterior del jugador",rem.error);else invalidateSignedUrl(previousPhoto)}catch(error){console.warn("No se pudo borrar la foto anterior del jugador",error)}
      }
...
let realtimeRefreshTimer=null,realtimeBurstStartedAt=0;
function scheduleRealtimeRefresh(){
  if(!realtimeBurstStartedAt)realtimeBurstStartedAt=Date.now();
  clearTimeout(realtimeRefreshTimer);
  realtimeRefreshTimer=setTimeout(async()=>{
    realtimeRefreshTimer=null;
    const burstStarted=realtimeBurstStartedAt;realtimeBurstStartedAt=0;
    if(isHistoricalSeason())return;
    if(lastFullLoadStartedAt>=burstStarted){
      if(lastFullLoadCompletedAt>=lastFullLoadStartedAt)return;
      realtimeBurstStartedAt=burstStarted;scheduleRealtimeRefresh();return;
    }
    try{await loadAll();await window.sportsV2700?.load?.()}catch(error){console.warn("Actualización Realtime",error)}
  },1400);
}
function realtime(){
...
function renderDashboard(){
  ...
  recent.innerHTML=players.slice(0,6).map(p=>`
    <button class="dashboard-player-row" onclick="editPlayer('${p.id}')">
      <img ${storageImageAttributes(p.photo_path)} alt="Fotografía de ${esc(p.name||"jugador")}" onerror="this.onerror=null;this.src='${placeholderPhoto}'">
