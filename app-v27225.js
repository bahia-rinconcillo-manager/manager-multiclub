// CD San Bernabé Manager · V27.2.25 · multiequipo por identidad + compatibilidad de temporadas
...
async function loadTeamUserProfiles(){
  ...
  let mappingResult=await sb.rpc("admin_list_technical_user_teams_v27225");
...
async function technicalAvailableTeams(){
  ...
  // V27.2.25: resuelve por identidad de la persona, unificando registros duplicados
  // y equipos de temporada activa, equipos legacy sin season_id y enlaces antiguos.
  let result=await sb.rpc("technical_available_teams_v27225",{p_token:token});
  if(result.error){
    result=await sb.rpc("technical_available_teams_v27224",{p_token:token});
  }
  if(result.error){
...
async function setTechnicalActiveTeam(teamId){
  if(!teamId)return;
  const token=getTechnicalSessionToken();
  let result=await sb.rpc("technical_select_team_v27225",{p_token:token,p_team_id:teamId});
  if(result.error){
    result=await sb.rpc("technical_select_team_v27224",{p_token:token,p_team_id:teamId});
  }
  if(result.error){
