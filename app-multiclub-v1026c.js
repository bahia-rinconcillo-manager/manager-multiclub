// V1.0.26C · Acceso técnico seguro por snapshot de equipo y configuración de club
...
const MULTICLUB_BUILD_VERSION="V1.0.26C";
...
async function createSeasonRolloverSafetyBackup(){
  ...
  for(const table of SEASON_SAFETY_BACKUP_TABLES){
    backup.tables[table]=sanitizeBackupValue(await fetchSeasonBackupTable(table,{optional:["season_archives","club_calendar_matches","technical_users","technical_user_teams","clothing_items","clothing_inventory","player_custom_clothing_sizes","staff_custom_clothing_sizes","club_equipment_state","public_registration_links","sponsorship_payments"].includes(table)}));
  }
...
async function executeSeasonRolloverWizard(){
  ...
  try{
    ...
    await logActivity("temporada","temporada",activeClubSeason?.id||null,`Temporada abierta: ${activeClubSeason?.name||d.newName} · temporada anterior cerrada`);
