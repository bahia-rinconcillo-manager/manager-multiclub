// MANAGER MULTICLUB · BUILD V1.0.22 · Informes completos Multiclub
...
rPitches.onclick=()=>downloadCSV("usos_alquileres.csv",pitchUsage.map(x=>({Fecha:x.usage_date,Hora:x.usage_time||"",EquipoEntidad:x.team_name,Tipo:x.usage_type,CampoPista:x.field_name,Importe:x.amount,Estado:pitchIsCancelled(x)?"Anulado":"Activo"})));
rSponsorships?.addEventListener("click",()=>downloadCSV("patrocinios.csv",(sponsorships||[]).map(s=>({Patrocinador:s.sponsor_name,Tipo:s.sponsorship_type,Duracion:s.duration,ImporteTotal:Number(s.amount||0),Pagado:Number(s.paid_amount||0),Pendiente:Math.max(0,Number(s.amount||0)-Number(s.paid_amount||0)),Estado:s.status||"Pendiente",Contacto:s.contact_name||"",Telefono:s.contact_phone||"",Email:s.contact_email||"",Observaciones:s.notes||""}))));
