...
const MULTICLUB_BUILD_VERSION="V1.0.35";
...
const SPORTS_TABLES_V1032=new Set([
  "sports_training_sessions","sports_training_attendance","sports_matches","sports_match_player_stats","club_calendar_matches"
]);
let realtimePendingTablesV1032=new Set();
let realtimeTimerV1032=null;
let realtimeFlushPromiseV1032=null;
let realtimeNeedsAnotherFlushV1032=false;
let realtimeSignalsV1032=0;
let realtimeBatchesV1032=0;

function sportsUiVisibleV1032(){
...
async function flushRealtimeV1032(){
  if(realtimeFlushPromiseV1032){realtimeNeedsAnotherFlushV1032=true;return realtimeFlushPromiseV1032}
  if(!realtimePendingTablesV1032.size||isHistoricalSeason())return false;
  const tables=new Set(realtimePendingTablesV1032);
  realtimePendingTablesV1032.clear();
  realtimeBatchesV1032++;
  realtimeFlushPromiseV1032=(async()=>{
    if(tables.has("club_seasons")){
    ...
    const onlySeason=tables.size===1&&tables.has("club_seasons");
    if(!onlySeason)await loadAll();
    if([...tables].some(t=>SPORTS_TABLES_V1032.has(t))){
      sportsLoadedV1032=false;
      if(sportsUiVisibleV1032())await ensureSportsLoadedV1032();
    }
    return true;
  })().finally(()=>{
    realtimeFlushPromiseV1032=null;
    if(realtimeNeedsAnotherFlushV1032||realtimePendingTablesV1032.size){
      realtimeNeedsAnotherFlushV1032=false;
      scheduleRealtimeFlushV1032();
    }
  });
  return realtimeFlushPromiseV1032;
}
function scheduleRealtimeFlushV1032(){
  if(realtimeTimerV1032)return;
  realtimeTimerV1032=setTimeout(()=>{
    realtimeTimerV1032=null;
    flushRealtimeV1032().catch(error=>console.warn("Multiclub V1.0.32 · Realtime",error));
  },140);
}
function noteRealtimeV1032(table){
  if(isHistoricalSeason())return;
  realtimeSignalsV1032++;
  realtimePendingTablesV1032.add(String(table||""));
  scheduleRealtimeFlushV1032();
}
function realtime(){
  ...
  for(const table of REALTIME_TABLES_V1032){
    next=next.on("postgres_changes",{
      event:"*",schema:"public",table,filter:`club_id=eq.${clubId}`
    },()=>noteRealtimeV1032(table));
  }
  channel=next.subscribe();
  window.MULTICLUB_REALTIME_V1032=Object.freeze({
    active:true,
    clubId,
    metrics:()=>({
      signals:realtimeSignalsV1032,
      batches:realtimeBatchesV1032,
      pending:[...realtimePendingTablesV1032],
      refreshing:!!realtimeFlushPromiseV1032
    }),
    refresh:()=>{realtimePendingTablesV1032.add("manual");return flushRealtimeV1032()}
  });
  return channel;
}
