/* CD San Bernabé Manager V27.2.2 · Tarjetero RFAF multitemporada */
(() => {
  "use strict";
        ...
        <div class="team-cards-layout">
          ...
          <main class="team-card-main">
            ...
            <div class="team-card-actions">
              ...
              ${(context.technical || historical()) ? "" : `<button type="button" class="danger-mini" data-card-delete>Eliminar ficha actual</button>`}
            </div>
          </main>
        </div>
      </section>`;
  ...
  async function deleteCurrentCard() {
    if (!activeContext || activeContext.technical) return;
    if (historical()) return notify("La temporada histórica es de solo lectura");
    const state = getState(activeContext.teamId);
    const card = state.cards[state.index];
    if (!card) return;
    if (!confirm(`¿Eliminar la ficha de ${card.player_name || "este jugador"}?\n\nNo se eliminará el jugador.`)) return;
    const { error } = await activeContext.client.from("team_player_cards").delete().eq("id", card.id);
