...
const BASE_CLOTHING_ITEMS=[
  {key:"game_shirt",label:"Camiseta equipación",icon:"👕",category:"Juego",audience:"players",custom:false},
  {key:"game_shorts",label:"Pantalón equipación",icon:"🩳",category:"Juego",audience:"players",custom:false},
  {key:"socks",label:"Medias",icon:"🧦",category:"Juego",audience:"players",custom:false},
  {key:"training_shirt",label:"Camiseta entrenamiento",icon:"👕",category:"Entrenamiento",audience:"both",custom:false},
  {key:"training_shorts",label:"Pantalón entrenamiento",icon:"🩳",category:"Entrenamiento",audience:"both",custom:false},
...
// ================================================================
// V27.2.4 · Gestión multitemporada con asistente de cambio anual
...
// ================================================================
const SEASON_SELECTED_KEY="cdsb_selected_season_v2724";
let clubSeasons=[];
...
function seasonTeamCategory(teamName){
  ...
  return normalizeSeasonCategory(team?.category||team?.name||teamName);
}
function teamsForSeasonCategory(category){
  if(!category)return[];
  return sortTeamsAlpha(teams).filter(t=>normalizeSeasonCategory(t?.category||t?.name)===category).map(t=>String(t?.name||"").trim()).filter(Boolean);
}
function seasonPlayerAutomaticTeam(item,seasonName){
  ...
  if(!suggested)return "";
  if(currentCategory===suggested&&String(item.currentTeam||"").trim())return String(item.currentTeam).trim();
  const candidates=teamsForSeasonCategory(suggested);
...
function recalculateSeasonPlayerSuggestions(){
  ...
  d.players.forEach(x=>{
    ...
    x.autoTargetTeam=seasonPlayerAutomaticTeam(x,d.newName);
    if(!x.teamTouched)x.targetTeam=x.autoTargetTeam;
  });
}
function seasonTargetTeamOptions(selected,includePlaceholder=true){
  const names=[...new Set(sortTeamsAlpha(teams).map(t=>String(t?.name||"").trim()).filter(Boolean))];
  const current=String(selected||"").trim();
  if(current&&!names.includes(current))names.unshift(current);
  return `${includePlaceholder?'<option value="">— Seleccionar equipo —</option>':""}${names.map(name=>`<option value="${esc(name)}" ${name===current?"selected":""}>${esc(name)}</option>`).join("")}`;
}
function initSeasonRolloverDraft(){
  ...
  const playerPlan=sortByAlpha((players||[]).filter(p=>!p.deleted_at),p=>`${p.team||""} ${p.surname||""} ${p.name||""}`).map(p=>{
    const item={
      id:p.id,
    ...
    item.autoTargetTeam=seasonPlayerAutomaticTeam(item,newName);
    item.targetTeam=item.autoTargetTeam;
    return item;
  });
  seasonRolloverDraft={
    step:1,
    ...
    keepSizes:true,
    players:playerPlan,
    staff:sortByAlpha(staff||[],m=>`${m.team_name||""} ${m.name||""}`).map(m=>({
      id:m.id,name:m.name||"",role:m.role||"",currentTeam:m.team_name||"",targetTeam:m.team_name||"",keep:true
    })),
    confirmation:"",
    executing:false
  };
...
function renderSeasonWizardSteps(){
  ...
  const labels=["Nueva temporada","Jugadores","Cuerpo técnico","Revisión final"];
...
function renderSeasonPlayerRows(){
  ...
  const categoryFilters=[...new Set(d.players.map(x=>x.suggestedCategory||"Revisar"))].sort((a,b)=>a.localeCompare(b,"es",{sensitivity:"base"}));
  const visible=d.players.filter(x=>(!seasonPlayerFilter||(x.currentTeam||"Sin equipo")===seasonPlayerFilter)&&(!seasonPlayerCategoryFilter||(x.suggestedCategory||"Revisar")===seasonPlayerCategoryFilter));
  return `<div class="season-wizard-toolbar season-player-toolbar">
    <label>Equipo actual<select id="seasonPlayerFilter"><option value="">Todos los equipos</option>${filters.map(x=>`<option ${x===seasonPlayerFilter?"selected":""}>${esc(x)}</option>`).join("")}</select></label>
    <label>Categoría ${esc(d.newName)}<select id="seasonPlayerCategoryFilter"><option value="">Todas</option>${categoryFilters.map(x=>`<option ${x===seasonPlayerCategoryFilter?"selected":""}>${esc(x)}</option>`).join("")}</select></label>
    <label>Asignar visibles a<select id="seasonPlayerBulkTeam"><option value="">Seleccionar equipo</option>${seasonTargetTeamOptions(seasonPlayerBulkTeam,false)}</select></label>
  ...
  <div class="season-wizard-info season-age-info"><strong>Asignación inteligente:</strong> el programa calcula la categoría orientativa por año de nacimiento. Si un jugador cambia de categoría y hay más de un equipo posible (por ejemplo Infantil A/B), queda <b>pendiente</b> para que elijas su equipo. No se cambiará nada hasta el cierre definitivo.</div>
  <div class="season-wizard-table-wrap"><table class="season-wizard-table season-player-table"><thead><tr><th>Continúa</th><th>Jugador</th><th>Nac.</th><th>Equipo ${esc(activeClubSeason?.name||"")}</th><th>Categoría ${esc(d.newName)}</th><th>Equipo ${esc(d.newName)}</th></tr></thead><tbody>
    ${visible.map(x=>`<tr class="${x.keep?(!String(x.targetTeam||"").trim()?"season-row-review":""):"season-row-drop"}"><td><input type="checkbox" data-season-player-keep="${esc(x.id)}" ${x.keep?"checked":""}></td><td><strong>${esc(x.name)}</strong></td><td>${esc(x.birthYear||"-")}</td><td>${esc(x.currentTeam||"Sin equipo")}</td><td><span class="season-category-badge ${x.suggestedCategory?"":"review"}">${esc(x.suggestedCategory||"REVISAR")}</span></td><td><select data-season-player-team="${esc(x.id)}" ${x.keep?"":"disabled"}>${seasonTargetTeamOptions(x.targetTeam)}</select>${x.keep&&!String(x.targetTeam||"").trim()?'<small class="season-pending-text">Pendiente de asignar</small>':""}</td></tr>`).join("")||'<tr><td colspan="6">No hay jugadores en este filtro.</td></tr>'}
  </tbody></table></div>`;
}
function renderSeasonStaffRows(){
  ...
  return `<div class="season-wizard-toolbar">
    <label>Equipo actual<select id="seasonStaffFilter"><option value="">Todos los equipos</option>${filters.map(x=>`<option ${x===seasonStaffFilter?"selected":""}>${esc(x)}</option>`).join("")}</select></label>
...
function renderSeasonWizard(){
  ...
  if(seasonWizardNext){seasonWizardNext.disabled=d.executing;seasonWizardNext.textContent=d.step===4?`Cerrar ${activeClubSeason?.name||"temporada"} y abrir ${d.newName}`:"Continuar"}
  if(seasonWizardHint)seasonWizardHint.textContent=d.executing?"Guardando histórico y creando la nueva temporada…":`Paso ${d.step} de 4`;
  if(d.step===1){
    ...
    seasonWizardBody.innerHTML=`<div class="season-wizard-intro"><h3>2. Decide qué jugadores continúan</h3><p>Marca las bajas y asigna a cada jugador que continúa el equipo de ${esc(d.newName)}. El histórico conservará su equipo anterior.</p></div>
      <div class="season-wizard-stats"><span><b>${c.playersTotal}</b> actuales</span><span class="ok"><b>${c.playersKeep}</b> continúan</span><span class="bad"><b>${c.playersDrop}</b> bajas</span><span class="pending"><b>${c.playersPending}</b> pendientes de equipo</span></div>${renderSeasonPlayerRows()}`;
  }else if(d.step===3){
    seasonWizardBody.innerHTML=`<div class="season-wizard-intro"><h3>3. Revisa el cuerpo técnico</h3><p>Puedes mantener, dar de baja o cambiar de equipo a entrenadores, delegados y demás miembros.</p></div>
      <div class="season-wizard-stats"><span><b>${c.staffTotal}</b> actuales</span><span class="ok"><b>${c.staffKeep}</b> continúan</span><span class="bad"><b>${c.staffDrop}</b> bajas</span></div>${renderSeasonStaffRows()}<div class="season-wizard-info"><strong>Importante sobre Usuarios:</strong> las credenciales de acceso son independientes del listado de cuerpo técnico. Al terminar el cambio anual revisa el apartado Usuarios para desactivar accesos de personas que ya no sigan en el club.</div>`;
  }else{
    const confirmation=`CERRAR ${activeClubSeason?.name||currentSeasonLabel()}`;
    const playerMoves=d.players.filter(x=>x.keep&&x.targetTeam!==x.currentTeam).length;
    const staffMoves=d.staff.filter(x=>x.keep&&x.targetTeam!==x.currentTeam).length;
    seasonWizardBody.innerHTML=`<div class="season-wizard-intro"><h3>4. Revisión final</h3><p>Esta es la última pantalla antes de realizar el cambio anual.</p></div>
      <div class="season-review-grid">
        ...
        <article><small>JUGADORES</small><strong>${c.playersKeep} continúan · ${c.playersDrop} bajas</strong><span>${playerMoves} cambio(s) de equipo.</span></article>
        <article><small>CUERPO TÉCNICO</small><strong>${c.staffKeep} continúan · ${c.staffDrop} bajas</strong><span>${staffMoves} cambio(s) de equipo.</span></article>
      ...
      <div class="season-final-warning"><strong>Las fichas RFAF no se trasladan como vigentes.</strong> Se archivarán en ${esc(activeClubSeason?.name||"")} y deberás importar las nuevas licencias en ${esc(d.newName)}.</div>
    ...
    if(seasonWizardNext)seasonWizardNext.disabled=d.confirmation.trim()!==confirmation||d.executing;
  }
}
function validateSeasonWizardStep(){
  ...
  if(d.step===2){
    const missing=d.players.filter(x=>x.keep&&!String(x.targetTeam||"").trim());
    if(missing.length)return toast(`Hay ${missing.length} jugador(es) que continúan pendientes de asignar a un equipo`),false;
  }
  if(d.step===3){
    const missing=d.staff.filter(x=>x.keep&&!String(x.targetTeam||"").trim());
    if(missing.length)return toast(`Hay ${missing.length} miembro(s) del cuerpo técnico sin equipo de destino`),false;
  }
  return true;
}
async function executeSeasonRolloverWizard(){
  ...
  try{
    ...
    const {data,error}=await sb.rpc("cdsb_rollover_season_v2722",{
      p_current_season_id:activeClubSeason.id,
      p_new_name:d.newName,
      p_snapshot:snapshot,
      p_player_plan:d.players.map(x=>({id:x.id,keep:Boolean(x.keep),team:String(x.targetTeam||x.currentTeam||"").trim()})),
      p_staff_plan:d.staff.map(x=>({id:x.id,keep:Boolean(x.keep),team_name:String(x.targetTeam||x.currentTeam||"").trim()})),
    ...
    toast(`Temporada ${activeClubSeason.name} abierta correctamente`);
...
function seasonWizardVisiblePlayers(){
  ...
  return seasonRolloverDraft.players.filter(x=>(!seasonPlayerFilter||(x.currentTeam||"Sin equipo")===seasonPlayerFilter)&&(!seasonPlayerCategoryFilter||(x.suggestedCategory||"Revisar")===seasonPlayerCategoryFilter));
}
...
seasonWizardBody?.addEventListener("change",event=>{
  ...
  if(t.dataset.seasonPlayerKeep){const x=d.players.find(v=>String(v.id)===String(t.dataset.seasonPlayerKeep));if(x){x.keep=t.checked;if(x.keep&&!x.targetTeam&&!x.teamTouched)x.targetTeam=x.autoTargetTeam||""}renderSeasonWizard();return}
  if(t.dataset.seasonPlayerTeam){const x=d.players.find(v=>String(v.id)===String(t.dataset.seasonPlayerTeam));if(x){x.targetTeam=t.value;x.teamTouched=true}renderSeasonWizard();return}
  ...
  if(t.dataset.seasonStaffTeam){const x=d.staff.find(v=>String(v.id)===String(t.dataset.seasonStaffTeam));if(x)x.targetTeam=t.value;return}
});
seasonWizardBody?.addEventListener("input",event=>{
  const d=seasonRolloverDraft;if(!d)return;
  if(event.target.id==="wizardNewSeasonName"){d.newName=event.target.value.trim();if(seasonWizardTitle)seasonWizardTitle.textContent=`${activeClubSeason?.name||currentSeasonLabel()} → ${d.newName||"siguiente temporada"}`}
  if(event.target.id==="seasonWizardConfirmation"){
    d.confirmation=event.target.value;
    const exact=`CERRAR ${activeClubSeason?.name||currentSeasonLabel()}`;
    if(seasonWizardNext)seasonWizardNext.disabled=d.confirmation.trim()!==exact||d.executing;
  }
});
seasonWizardBody?.addEventListener("click",event=>{
  ...
  if(playerAction){
    ...
    if(playerAction==="assign"){
      if(!seasonPlayerBulkTeam)return toast("Selecciona primero el equipo de destino para los jugadores visibles");
      visible.filter(x=>x.keep).forEach(x=>{x.targetTeam=seasonPlayerBulkTeam;x.teamTouched=true});
    }else if(playerAction==="keep"){
      visible.forEach(x=>{x.keep=true;if(!x.targetTeam&&!x.teamTouched)x.targetTeam=x.autoTargetTeam||""});
...
seasonWizardNext?.addEventListener("click",async()=>{
  ...
  if(d.step<4){
    ...
    if(d.step===1)recalculateSeasonPlayerSuggestions();
...
function show(id){["setupScreen","authScreen","app"].forEach(x=>document.getElementById(x).classList.add("hidden"));document.getElementById(id).classList.remove("hidden")}
if(!initClient())show("setupScreen");else checkSession();
...
loginForm.onsubmit=async e=>{
  ...
  }catch(error){authMessage.textContent=error.message||"No se pudo iniciar sesión."}
...
async function loadTrashAndActivity(){
  const [p,t,a]=await Promise.all([
    ...
    sb.from("teams").select("*").not("deleted_at","is",null).order("deleted_at",{ascending:false}),
...
async function loadAll(){
  ...
  const queries=await Promise.allSettled([
    ...
    sb.from("kits").select("*"),
    sb.from("teams").select("*").is("deleted_at",null).order("name"),
  ...
  teamsGrid.innerHTML=orderedTeams.length?orderedTeams.map(t=>{
    ...
    return `<article class="team-card">
      <div class="team-card-top">
        <div><h4>${esc(t.name)}</h4><p>${esc(t.category||"Sin categoría")}</p></div>
...
function renderCalendar(){
 ...
 calendarList.innerHTML=events.length?events.map(e=>{const d=new Date(e.event_date+"T12:00:00");return `<article class="event-card"><div class="event-date"><span>${String(d.getDate()).padStart(2,"0")}</span><small>${months[d.getMonth()]}</small></div><div class="event-info"><h4>${esc(e.title)}</h4><p>${esc(e.event_time||"Sin hora")} · ${esc(e.location||"Sin lugar")}${e.team_name?" · "+esc(e.team_name):""}</p><span class="event-type">${esc(e.event_type)}</span></div><button class="mini" onclick="editEvent('${e.id}')">Editar</button></article>`}).join(""):"No hay eventos creados.";
}
...
window.editTeam=id=>{
  ...
  ["id","name","category","delegate_name","training_schedule","field","notes"].forEach(name=>{
...
teamForm.onsubmit=async event=>{
  ...
  const data={name:(raw.name||"").trim(),category:(raw.category||"").trim()||null,coach_id:raw.coach_id||null,delegate_name:(raw.delegate_name||"").trim()||null,training_schedule:(raw.training_schedule||"").trim()||null,field:(raw.field||"").trim()||null,notes:(raw.notes||"").trim()||null};
  try{
...
const PLAYER_SIZES=["","3XS","2XS","XS","S","M","L","XL","2XL","3XL"];
const SOCK_SIZES=["","S","M","L"];
const STAFF_SIZES=["","S","M","L","XL","2XL","3XL"];
function fillSelect(select,values){select.innerHTML=values.map(v=>`<option value="${v}">${v||"Seleccionar"}</option>`).join("")}
function sizeFor(id){return {...(playerSizes.find(x=>x.player_id===id)||{}),...customClothingSizesFor("players",id)}}
function staffSizeFor(id){
  ...
  return {...base,...custom,sized:status.sized||(hasSize?"Sí":"No"),delivered:status.delivered||"No"};
}
function renderDashboardExtras(){
 const ym=new Date().toISOString().slice(0,7);
 stPitches.textContent=euro(pitchUsage.filter(x=>x.usage_date?.startsWith(ym)).reduce((s,x)=>s+Number(x.amount||0),0));
 const inc=financeMovements.filter(x=>x.movement_type==="Ingreso").reduce((s,x)=>s+Number(x.amount||0),0);
 const exp=financeMovements.filter(x=>x.movement_type==="Gasto").reduce((s,x)=>s+Number(x.amount||0),0);
...
function kitOrderRows(){
  ...
  const add=(personType,name,teamName,sizes,fields)=>{
    ...
    if(onlySized&&String(sizes.sized||"").toLowerCase()!=="sí"&&String(sizes.sized||"").toLowerCase()!=="si")return;
    fields.forEach(([key,defaultLabel])=>{
      const size=String(sizes[key]||"").trim();if(!size)return;
      const item=CLOTHING_ITEMS.find(x=>x.key===key),label=item?clothingConfig(item).label:defaultLabel;
  ...
  const playerFields=activeClothingItemsFor("players").map(item=>[item.key,clothingConfig(item).label]);
  const staffFields=activeClothingItemsFor("staff").map(item=>[item.key,clothingConfig(item).label]);
...
function kitOrderSummary(){
  ...
  return [...map.values()].sort((a,b)=>a.Prenda.localeCompare(b.Prenda,"es")||a.Talla.localeCompare(b.Talla,"es",{numeric:true}));
}
...
function clothingTableHeader(audience){
  return activeClothingItemsFor(audience).map(item=>`<th>${esc(clothingConfig(item).label)}</th>`).join("");
}
function clothingTableCells(audience,sizes){
  return activeClothingItemsFor(audience).map(item=>`<td>${esc(sizes[item.key]||"-")}</td>`).join("");
}
...
window.openKit=id=>{const s=sizeFor(id),k=kit(id);kitForm.reset();kitForm.player_id.value=id;
 ["game_shirt","game_shorts","training_shirt","training_shorts","training_sweatshirt","tracksuit_jacket","tracksuit_trousers"].forEach(n=>{fillSelect(kitForm.elements[n],PLAYER_SIZES);kitForm.elements[n].value=s[n]||""});
 fillSelect(kitForm.elements.socks,SOCK_SIZES);kitForm.elements.socks.value=s.socks||"";
...
kitForm.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(kitForm));const pid=d.player_id;
 const sizeData={player_id:pid,game_shirt:d.game_shirt,game_shorts:d.game_shorts,socks:d.socks,training_shirt:d.training_shirt,training_shorts:d.training_shorts,training_sweatshirt:d.training_sweatshirt,tracksuit_jacket:d.tracksuit_jacket,tracksuit_trousers:d.tracksuit_trousers,sized:d.sized,sizing_date:d.sizing_date||null,notes:d.notes};
...
function renderPitches(){
 const ym=pitchMonth.value||new Date().toISOString().slice(0,7),tf=pitchTeamFilter.value;
 pitchMonth.value=ym;pitchTeamFilter.innerHTML='<option value="">Todos los equipos</option>'+sortTeamsAlpha(teams).map(t=>`<option>${t.name}</option>`).join("");pitchTeamFilter.value=tf;
 const list=pitchUsage.filter(x=>(!ym||x.usage_date?.startsWith(ym))&&(!tf||x.team_name===tf));
 pitchTrainings.textContent=list.filter(x=>x.usage_type==="Entrenamiento").length;pitchMatches.textContent=list.filter(x=>x.usage_type==="Partido").length;pitchTotal.textContent=euro(list.reduce((s,x)=>s+Number(x.amount||0),0));
 pitchesBody.innerHTML=list.map(x=>`<tr><td>${new Date(x.usage_date+"T12:00:00").toLocaleDateString("es-ES")}</td><td>${x.team_name}</td><td>${x.usage_type}</td><td>${x.field_name||"-"}</td><td><strong>${euro(x.amount)}</strong></td><td><button class="mini" onclick="editPitch('${x.id}')">Editar</button></td></tr>`).join("")||'<tr><td colspan="6">Sin registros.</td></tr>'
}
pitchMonth.onchange=renderPitches;pitchTeamFilter.onchange=renderPitches;
addPitch.onclick=()=>{pitchForm.reset();pitchForm.usage_date.value=new Date().toISOString().slice(0,10);pitchForm.team_name.innerHTML=sortTeamsAlpha(teams).map(t=>`<option>${t.name}</option>`).join("");pitchForm.amount.value="12.50";pitchDialog.showModal()}
pitchForm.usage_type.onchange=()=>pitchForm.amount.value=pitchForm.usage_type.value==="Partido"?"25.00":"12.50";
window.editPitch=id=>{const x=pitchUsage.find(v=>v.id===id);pitchForm.reset();pitchForm.team_name.innerHTML=sortTeamsAlpha(teams).map(t=>`<option>${t.name}</option>`).join("");Object.entries(x).forEach(([k,v])=>{if(pitchForm.elements[k])pitchForm.elements[k].value=v??""});pitchDialog.showModal()}
pitchForm.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(pitchForm));d.amount=Number(d.amount);const id=d.id;delete d.id;try{const r=id?await sb.from("pitch_usage").update(d).eq("id",id):await sb.from("pitch_usage").insert(d);if(r.error)throw r.error;pitchDialog.close();toast("Uso registrado");await loadAll()}catch(error){showOperationError("No se pudo guardar el uso de pista.",error)}}

function renderFinance(){
 const ym=financeMonth.value||"",tf=financeTypeFilter.value;
 const list=financeMovements.filter(x=>(!ym||x.movement_date?.startsWith(ym))&&(!tf||x.movement_type===tf));
...
rSizes.onclick=()=>downloadCSV("tallajes_jugadores.csv",sortPlayersAlpha(players).map(p=>{const s=sizeFor(p.id);return {Jugador:`${p.name} ${p.surname}`,Equipo:p.team,CamisetaJuego:s.game_shirt,PantalonJuego:s.game_shorts,Medias:s.socks,CamisetaEntreno:s.training_shirt,PantalonEntreno:s.training_shorts,Sudadera:s.training_sweatshirt,ChaquetaChandal:s.tracksuit_jacket,PantalonChandal:s.tracksuit_trousers}}));
if(window.rStaffSizes)rStaffSizes.onclick=()=>downloadCSV("tallajes_cuerpo_tecnico.csv",sortStaffAlpha(staff).map(s=>{const z=staffSizeFor(s.id);return {Nombre:s.name,Cargo:s.role,Equipo:s.team_name,CamisetaEntreno:z.training_shirt,PantalonEntreno:z.training_shorts,Sudadera:z.training_sweatshirt,ChaquetaChandal:z.tracksuit_jacket,PantalonChandal:z.tracksuit_trousers,Polo:z.polo}}));
...
rPitches.onclick=()=>downloadCSV("pistas.csv",pitchUsage.map(x=>({Fecha:x.usage_date,Equipo:x.team_name,Tipo:x.usage_type,Campo:x.field_name,Importe:x.amount})));
rFinance.onclick=()=>downloadCSV("ingresos_gastos.csv",financeMovements.map(x=>({Fecha:x.movement_date,Tipo:x.movement_type,Categoria:x.category,Concepto:x.concept,Importe:x.amount,FormaPago:x.payment_method})));
...
function preparePlayerSizeSelects(values={}){
  ...
  ["game_shirt","game_shorts","training_shirt","training_shorts","training_sweatshirt","tracksuit_jacket","tracksuit_trousers"].forEach(name=>{
    const field=playerFormEl.elements[name];if(!field)return;fillSelect(field,PLAYER_SIZES);field.value=values[name]||"";
  });
  if(playerFormEl.elements.socks){fillSelect(playerFormEl.elements.socks,SOCK_SIZES);playerFormEl.elements.socks.value=values.socks||""}
}
...
if(playerFormEl)playerFormEl.onsubmit=async e=>{
  ...
  const sizeData={game_shirt:raw.game_shirt||null,game_shorts:raw.game_shorts||null,socks:raw.socks||null,training_shirt:raw.training_shirt||null,training_shorts:raw.training_shorts||null,training_sweatshirt:raw.training_sweatshirt||null,tracksuit_jacket:raw.tracksuit_jacket||null,tracksuit_trousers:raw.tracksuit_trousers||null};

  try{
    if(!playerData.name||!playerData.surname)throw new Error("Nombre y apellidos son obligatorios");
    if(!playerData.team)throw new Error("Selecciona un equipo");
...
async function createFullBackup(){
  ...
  try{
    ...
    const backup={application:"CD San Bernabé Manager",version:"V27.2.4",created_at:new Date().toISOString(),tables:{}};
...
window.renderTeamDetailTab=function(tab){
  ...
  if(tab==="overview") body=`
    <div class="team-detail-hero">
      <div><small>${esc(t.category||"Sin categoría")}</small><h3>${esc(t.name)}</h3><p>${esc(t.training_schedule||"Horario sin definir")} · ${esc(t.field||"Campo sin definir")}</p></div>
  ...
  if(tab==="sizes"){
    ...
    const sizeRows=roster.map(p=>{
      ...
      const garmentCells=sizeItems.map(item=>`<td>${esc(sizes[item.key]||"-")}</td>`).join("");
  ...
  teamDetailContent.innerHTML=`<nav class="team-detail-tabs">${tabs.map(([id,label])=>`<button class="${tab===id?"active":""} ${id.startsWith("sports")?"sports-tab":""} ${id==="cards"?"cards-tab":""}" onclick="renderTeamDetailTab('${id}')">${id==="cards"?"▣ ":id==="sportsAttendance"?"✓ ":id==="sportsMatches"?"⚽ ":id==="sportsStats"?"▥ ":""}${label}</button>`).join("")}</nav><div class="team-detail-tab-content">${body}</div>`;
  if(tab==="cards"){
...
function teamReportRows(){
  ...
  if(cfg.type==="sizes"){
    headers=["Jugador","Camiseta juego","Pantalón juego","Medias","Camiseta entreno","Pantalón entreno","Sudadera","Chaqueta chándal","Pantalón chándal","Tallado","Entregado"];
    if(cfg.notes)headers.push("Observaciones");
    ...
    list.forEach(p=>{const s=sizeFor(p.id),k=kit(p.id);const row=[
      `${p.surname||""}, ${p.name||""}`.replace(/^, /,""),
      reportText(s.game_shirt),reportText(s.game_shorts),reportText(s.socks),
      reportText(s.training_shirt),reportText(s.training_shorts),reportText(s.training_sweatshirt),
      reportText(s.tracksuit_jacket),reportText(s.tracksuit_trousers),
  ...
  if(cfg.type==="full"){
    ...
    list.forEach(p=>{const s=sizeFor(p.id),k=kit(p.id);const row=[
      ...
      [s.game_shirt,s.game_shorts,s.socks].filter(Boolean).join(" / ")||"-",
      [s.training_shirt,s.training_shorts,s.training_sweatshirt].filter(Boolean).join(" / ")||"-",
...
function saveClubMatches(){localStorage.setItem(matchesSeasonKey(),JSON.stringify(clubMatches))}
...
window.deleteClubMatch=id=>{const m=clubMatches.find(x=>x.id===id);if(!m||!confirm(`¿Eliminar el partido contra ${m.opponent}?`))return;clubMatches=clubMatches.filter(x=>x.id!==id);saveClubMatches();renderClubMatches();renderDashboardMatches();if(currentTeamDetailId)renderTeamDetailTab("calendar");toast("Partido eliminado")};
function saveMatchForm(event){
  ...
  saveClubMatches();document.getElementById("matchDialog")?.close();renderClubMatches();renderDashboardMatches();if(currentTeamDetailId)renderTeamDetailTab("calendar");toast(i>=0?"Partido actualizado":"Partido añadido");
}
...
function saveResultForm(event){event.preventDefault();const raw=Object.fromEntries(new FormData(event.currentTarget)),m=clubMatches.find(x=>x.id===raw.id);if(!m)return;m.goals_for=Number(raw.goals_for);m.goals_against=Number(raw.goals_against);saveClubMatches();document.getElementById("resultDialog")?.close();renderClubMatches();renderDashboardMatches();if(currentTeamDetailId)renderTeamDetailTab("calendar");toast("Resultado guardado")}
window.toggleMatchFinished=id=>{const m=clubMatches.find(x=>x.id===id);if(!m)return;if(!matchIsPlayed(m)&&!m.finished)return toast("Introduce primero el resultado");m.finished=!m.finished;saveClubMatches();renderClubMatches();renderDashboardMatches();if(currentTeamDetailId)renderTeamDetailTab("calendar");toast(m.finished?"Partido finalizado":"Partido reabierto")};
...
async function extractPdfLines(file){
  if(!window.pdfjsLib)throw new Error("No se pudo cargar el lector de PDF. Comprueba la conexión a internet y vuelve a intentarlo.");
  window.pdfjsLib.GlobalWorkerOptions.workerSrc="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js";
  const data=await file.arrayBuffer(),pdf=await window.pdfjsLib.getDocument({data}).promise,lines=[];
  for(let pageNum=1;pageNum<=pdf.numPages;pageNum++){
    const page=await pdf.getPage(pageNum),content=await page.getTextContent(),groups=new Map();
    content.items.forEach(item=>{const y=Math.round(item.transform?.[5]||0);if(!groups.has(y))groups.set(y,[]);groups.get(y).push({x:item.transform?.[4]||0,text:item.str||""})});
    [...groups.entries()].sort((a,b)=>b[0]-a[0]).forEach(([,items])=>lines.push(items.sort((a,b)=>a.x-b.x).map(i=>i.text).join(" ")));
  }
  return lines;
}
function renderPdfPreview(){
  ...
  body.innerHTML=pdfParsedMatches.length?pdfParsedMatches.map((m,i)=>`<tr>
    <td><input type="checkbox" data-pdf-field="selected" data-index="${i}" ${m.selected?"checked":""}></td>
    ...
    <td><input data-pdf-field="location" data-index="${i}" value="${esc(m.location)}"></td>
  </tr>`).join(""):'<tr><td colspan="7">Todavía no hay partidos detectados.</td></tr>';
  button.disabled=!pdfParsedMatches.some(m=>m.selected);
  body.querySelectorAll("input").forEach(input=>input.addEventListener("change",()=>{const m=pdfParsedMatches[Number(input.dataset.index)],field=input.dataset.pdfField;if(!m)return;m[field]=input.type==="checkbox"?input.checked:input.value;if(field==="home"||field==="away"){const team=(document.getElementById("pdfImportTeam")?.value||"").toLowerCase();m.venue=String(m.away||"").toLowerCase().includes(team)?"away":"home";m.opponent=m.venue==="away"?m.home:m.away}button.disabled=!pdfParsedMatches.some(x=>x.selected)}));
}
async function handlePdfFile(){
  const file=document.getElementById("matchesPdfFile")?.files?.[0],team=document.getElementById("pdfImportTeam")?.value||"",competition=document.getElementById("pdfImportCompetition")?.value.trim()||"";
  const status=document.getElementById("matchesPdfStatus");if(!file)return;if(!team){toast("Selecciona el equipo antes de importar");document.getElementById("matchesPdfFile").value="";return}
  try{status.textContent="Leyendo el PDF y detectando partidos...";const lines=await extractPdfLines(file);pdfParsedMatches=parsePdfLines(lines,team,competition);status.textContent=pdfParsedMatches.length?`${pdfParsedMatches.length} partido(s) detectado(s). Revisa los datos antes de importar.`:"No se detectaron partidos automáticamente. El formato del PDF puede no ser compatible; puedes añadirlos manualmente.";renderPdfPreview()}
...
function confirmPdfImport(){
  const selected=pdfParsedMatches.filter(m=>m.selected);if(!selected.length)return;
  selected.forEach(m=>clubMatches.push({id:`match_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,team:m.team,competition:m.competition,round:m.round,match_date:m.match_date,match_time:m.match_time,venue:m.venue,opponent:m.opponent,location:m.location,goals_for:"",goals_against:"",notes:m.notes||"Importado desde PDF"}));
  saveClubMatches();document.getElementById("matchesPdfDialog")?.close();pdfParsedMatches=[];renderClubMatches();renderDashboardMatches();if(currentTeamDetailId)renderTeamDetailTab("calendar");toast(`${selected.length} partido(s) importado(s)`);
}
function renderDashboardMatches(){
  ...
  const card=m=>{const done=matchIsPlayed(m),date=new Date(m.match_date+"T12:00:00");return `<article class="dashboard-match-item"><time><strong>${date.toLocaleDateString("es-ES",{weekday:"short",day:"2-digit"})}</strong><span>${esc(m.match_time||"Sin hora")}</span></time><div><strong>${esc(m.team)}</strong><span>${esc(m.opponent)}</span><small>${esc(m.location||"Campo pendiente")}</small></div><div class="dashboard-match-score">${done?`${esc(m.goals_for)}-${esc(m.goals_against)}`:"Pendiente"}</div></article>`};
...
function setupMatchesModule(){
  ...
  document.getElementById("importMatchesPdf")?.addEventListener("click",()=>{fillMatchTeamSelects();pdfParsedMatches=[];document.getElementById("matchesPdfFile").value="";document.getElementById("matchesPdfStatus").textContent="";renderPdfPreview();document.getElementById("matchesPdfDialog")?.showModal()});
  document.getElementById("matchesPdfFile")?.addEventListener("change",handlePdfFile);
...
function saveRivalTeams(){localStorage.setItem(RIVAL_TEAMS_STORAGE_KEY,JSON.stringify(rivalTeams))}
function rivalByName(name){return rivalTeams.find(r=>String(r.name).trim().toLowerCase()===String(name||"").trim().toLowerCase())}
function crestHTML(name,club=false){const r=rivalByName(name),src=club?"assets/escudo-oficial.png":r?.crest;return src?`<span class="match-team-crest"><img src="${src}" alt="Escudo ${esc(name)}"></span>`:`<span class="match-team-crest fallback">${esc(String(name||"?").trim().charAt(0).toUpperCase())}</span>`}
...
competitionLogoHTML=function(name,compact=false){const src=COMPETITION_IMAGES[name],label=name||"Sin competición";return src?`<span class="competition-logo official ${compact?"compact":""}"><img src="${src}" alt="${esc(label)}"><small>${esc(label)}</small></span>`:`<span class="competition-logo ${compact?"compact":""}"><b>?</b><small>${esc(label)}</small></span>`};
...
window.openRivalTeamsManager=()=>{loadRivalTeams();resetRivalTeamForm();renderRivalTeams();document.getElementById("rivalTeamsDialog")?.showModal()};
...
async function saveRivalTeamForm(e){e.preventDefault();const f=e.currentTarget,id=f.elements.id.value,name=f.elements.name.value.trim(),file=f.elements.crest.files?.[0];if(!name)return toast("Escribe el nombre del equipo");let crest=id?rivalTeams.find(r=>r.id===id)?.crest||"":"";if(file)crest=await fileToDataURL(file);const duplicate=rivalTeams.find(r=>r.name.toLowerCase()===name.toLowerCase()&&r.id!==id);if(duplicate)return toast("Ese equipo ya está registrado");if(id){const i=rivalTeams.findIndex(r=>r.id===id);if(i>=0)rivalTeams[i]={...rivalTeams[i],name,crest}}else rivalTeams.push({id:`rival_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,name,crest});saveRivalTeams();resetRivalTeamForm();renderRivalTeams();populateOpponentSelect();toast(id?"Equipo actualizado":"Equipo rival añadido")}
...
// V25.0 · Usuarios técnicos sin email, sesión propia y acceso de solo lectura por equipo
...
async function loadTeamUserProfiles(){
  ...
  const {data,error}=await sb.rpc("admin_list_technical_users");
  if(error){
    teamUserProfiles=[];
    if(usersSetupWarning){usersSetupWarning.classList.remove("hidden");usersSetupWarning.textContent="Falta preparar el sistema de usuarios. Ejecuta en Supabase el archivo EJECUTAR_UNICO_SQL_V25_0.sql."}
  }else{
    teamUserProfiles=data||[];
...
function renderTeamUsers(){
  ...
  teamUsersBody.innerHTML=orderedProfiles.length?orderedProfiles.map(u=>{
    const team=teams.find(t=>String(t.id)===String(u.team_id));
...
window.resetTeamUserPassword=async id=>{
  ...
  try{
    ...
    const {data,error}=await sb.rpc("admin_save_technical_user",{p_id:u.id,p_username:u.username,p_password_hash:hash,p_password_salt:salt,p_password_iterations:iterations,p_initial_password:password,p_display_name:u.display_name,p_role_label:u.role_label,p_team_id:u.team_id,p_active:u.active!==false});
    ...
    // Actualiza inmediatamente el usuario en memoria para que WhatsApp, copiar e imprimir
    // dispongan de la nueva contraseña incluso antes de volver a consultar Supabase.
    u.initial_password=password;
    u.password_ready=true;
    u.failed_attempts=0;
    u.locked_until=null;
    renderTeamUsers();
    toast("Contraseña restablecida correctamente");
    await loadTeamUserProfiles();
    // Si la caché de PostgREST tarda en refrescarse, conserva el dato recién establecido.
...
window.printTeamUserCredentials=id=>{
  ...
  const team=teams.find(t=>String(t.id)===String(u.team_id));
  const w=window.open("","_blank","width=700,height=700");if(!w)return toast("El navegador bloqueó la ventana de impresión");
  w.document.write(`<html><head><title>Credenciales ${esc(u.display_name)}</title><style>body{font-family:Arial;padding:45px;color:#102a43}.card{border:3px solid #0b4da2;border-radius:18px;padding:35px;max-width:560px;margin:auto}h1{color:#0b4da2}.row{padding:12px 0;border-bottom:1px solid #ddd}.label{font-size:12px;color:#667;text-transform:uppercase}.value{font-size:22px;font-weight:700;margin-top:4px}.note{margin-top:25px;font-size:12px;color:#667}</style></head><body><div class="card"><h1>CD SAN BERNABÉ</h1><h2>Credenciales de acceso</h2><div class="row"><div class="label">Nombre</div><div class="value">${esc(u.display_name)}</div></div><div class="row"><div class="label">Equipo</div><div class="value">${esc(team?.name||"Sin equipo")}</div></div><div class="row"><div class="label">Cargo</div><div class="value">${esc(u.role_label||"Cuerpo técnico")}</div></div><div class="row"><div class="label">Usuario</div><div class="value">${esc(u.username)}</div></div><div class="row"><div class="label">Contraseña</div><div class="value">${esc(u.initial_password||"Restablecer desde el panel")}</div></div><div class="row"><div class="label">Dirección de acceso</div><div class="value" style="font-size:15px">${esc(location.origin)}</div></div><p class="note">Documento confidencial. No compartas estas credenciales con terceros.</p></div><script>window.onload=()=>window.print()<\/script></body></html>`);w.document.close();
};
...
function fillTeamUserTeams(selected=""){
  ...
  teamUserForm.elements.team_id.innerHTML='<option value="">Seleccionar equipo</option>'+sortTeamsAlpha(teams).map(t=>`<option value="${t.id}">${esc(t.name)}</option>`).join("");
  teamUserForm.elements.team_id.value=selected||"";
}
...
window.editTeamUser=id=>{
  const u=teamUserProfiles.find(x=>String(x.id)===String(id));if(!u)return;
  teamUserForm.reset();fillTeamUserTeams(u.team_id);
...
teamUserForm?.addEventListener("submit",async e=>{
  ...
  try{
    ...
    if(!raw.team_id)throw new Error("Selecciona un equipo");
    ...
    const {data,error}=await sb.rpc("admin_save_technical_user",{
      ...
      p_display_name:raw.display_name.trim(),p_role_label:raw.role_label,p_team_id:raw.team_id,p_active:raw.active==="true"
    });
    if(error)throw error;
    const result=Array.isArray(data)?data[0]:data;
...
function applyReadOnlyToTeamDetail(){
  ...
  root.querySelectorAll('button').forEach(btn=>{
    ...
    const keep=text.includes("informe")||text.includes("descargar")||btn.closest(".team-detail-tabs")||btn.closest(".team-sports-panel")||btn.closest(".team-cards-panel")||btn.dataset.sportsAction==="true";
    if(!keep)btn.remove();
  });
}
