...
(function(){
  ...
  async function load(){
    ...
    try{
      ...
      const missingSnapshot=String(error?.code||"")==="PGRST202" && /sports_snapshot/i.test(String(error?.message||""));
      if(missingSnapshot){
        // En Multiclub este RPC antiguo de San Bernabé no debe ejecutarse sin club_id:
        // se deja el seguimiento vacío hasta migrar el módulo deportivo de forma segura.
        trainings=[];trainingAttendance=[];matches=[];matchStats=[];loaded=true;
        initialiseFilters();render();renderActiveTeamPanel();
        console.info("Seguimiento deportivo pendiente de adaptación Multiclub; aviso antiguo ocultado.");
        return true;
      }
