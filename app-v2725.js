...
// ================================================================
// V27.2.5 · Gestión multitemporada con asistente de cambio anual
...
function seasonNewTeamsForCategory(category){
  ...
  if(!category)return[];
  return list.filter(t=>normalizeSeasonCategory(t.category||t.name)===category);
}
...
function seasonTargetTeamOptions(selectedId,includePlaceholder=true){
  ...
  return `${includePlaceholder?'<option value="">— Sin equipo / decidir después —</option>':""}${teamsList.map(team=>`<option value="${esc(team.draftId)}" ${String(team.draftId)===current?"selected":""}>${esc(team.name||"Equipo sin nombre")}${team.category?` · ${esc(team.category)}`:""}</option>`).join("")}`;
}
...
function renderSeasonNewTeams(){
  ...
  <div class="season-new-team-list">
    ${(d.newTeams||[]).map((team,index)=>`<article class="season-new-team-card">
      ...
      <label>Categoría<input data-season-new-team-category="${esc(team.draftId)}" value="${esc(team.category)}" placeholder="Infantil"></label>
...
function renderSeasonWizard(){
  ...
  if(seasonWizardNext){seasonWizardNext.disabled=d.executing;seasonWizardNext.textContent=d.step===5?`Cerrar ${activeClubSeason?.name||"temporada"} y abrir ${d.newName}`:"Continuar"}
  ...
  if(d.step===1){
    ...
    const playerMoves=d.players.filter(x=>x.keep&&seasonNewTeamName(x.targetTeamId)!==String(x.currentTeam||"").trim()).length;
    const staffMoves=d.staff.filter(x=>x.keep&&seasonNewTeamName(x.targetTeamId)!==String(x.currentTeam||"").trim()).length;
...
async function executeSeasonRolloverWizard(){
  ...
  try{
    ...
    const {data,error}=await sb.rpc("cdsb_rollover_season_v2725",{
      p_current_season_id:activeClubSeason.id,
      p_new_name:d.newName,
      p_snapshot:snapshot,
      p_team_plan:d.newTeams.map(t=>({
        name:String(t.name||"").trim(),
        category:String(t.category||"").trim(),
        delegate_name:String(t.delegate_name||"").trim(),
        training_schedule:String(t.training_schedule||"").trim(),
      ...
      p_player_plan:d.players.map(x=>({id:x.id,keep:Boolean(x.keep),team:seasonNewTeamName(x.targetTeamId)})),
      p_staff_plan:d.staff.map(x=>({id:x.id,keep:Boolean(x.keep),team_name:seasonNewTeamName(x.targetTeamId)})),
...
seasonWizardBody?.addEventListener("input",event=>{
  ...
  const teamId=t.dataset.seasonNewTeamName||t.dataset.seasonNewTeamCategory||t.dataset.seasonNewTeamSchedule||t.dataset.seasonNewTeamField;
  if(teamId){
...
seasonWizardBody?.addEventListener("click",event=>{
  ...
  if(teamAction==="add"){
    d.newTeams.push({draftId:seasonDraftTeamId(),name:"",category:"",delegate_name:"",training_schedule:"",field:"",notes:""});
...
teamForm.onsubmit=async event=>{
  ...
  const data={name:(raw.name||"").trim(),category:(raw.category||"").trim()||null,coach_id:raw.coach_id||null,delegate_name:(raw.delegate_name||"").trim()||null,training_schedule:(raw.training_schedule||"").trim()||null,field:(raw.field||"").trim()||null,notes:(raw.notes||"").trim()||null,season_id:activeClubSeason?.id||null};
  try{
...
async function createFullBackup(){
  ...
  try{
    ...
    const backup={application:"CD San Bernabé Manager",version:"V27.2.5",created_at:new Date().toISOString(),tables:{}};
