// CD San Bernabé Manager · V27.2.29 · acceso técnico estable con selector previo · multiequipo por identidad + compatibilidad de temporadas
...
async function checkSession(){
  // V27.2.29: una sesión técnica se resuelve ANTES que cualquier sesión de administrador.
  // El selector se muestra en una pantalla independiente, nunca sobre una app aún sin cargar.
  if(getTechnicalSessionToken()){
    try{
      await enterTechnicalAccessV27229();
      return;
    }catch(error){
...
function parseTechnicalBundleV27229(data){
...
async function readTechnicalBundleV27229(){
  const token=getTechnicalSessionToken();
  if(!token)throw new Error("No hay una sesión del cuerpo técnico activa.");
  const {data,error}=await sb.rpc("technical_access_bundle_v27226",{p_token:token});
  if(error)throw new Error("No se pudieron cargar los equipos del acceso técnico: "+(error.message||"error desconocido"));
  const bundle=parseTechnicalBundleV27229(data);
...
function renderTechnicalTeamScreenV27229(bundle,{switching=false}={}){
...
async function selectTechnicalTeamV27229(teamId,{switching=false}={}){
  const token=getTechnicalSessionToken();
  if(!token)throw new Error("La sesión del cuerpo técnico ha caducado.");
  const {data,error}=await sb.rpc("technical_select_team_v27226",{p_token:token,p_team_id:teamId});
  if(error)throw error;
  const result=parseTechnicalBundleV27229(data)||data||{};
...
async function enterTechnicalAccessV27229({switching=false}={}){
  const bundle=await readTechnicalBundleV27229();
  ...
  if(bundle.teams.length===1){
    await selectTechnicalTeamV27229(bundle.teams[0].team_id,{switching});
    return;
  }
  renderTechnicalTeamScreenV27229(bundle,{switching});
}
...
document.addEventListener("click",async event=>{
  ...
  if(button){
    ...
    try{
      ...
      await selectTechnicalTeamV27229(button.dataset.technicalEntryTeam,{switching:!!window.__technicalEntrySwitching});
    }catch(error){
...
switchTechnicalTeam?.addEventListener("click",async()=>{
  if(!isTechnicalReadOnly())return;
  try{
    await enterTechnicalAccessV27229({switching:true});
