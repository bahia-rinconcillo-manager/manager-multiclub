/* CD San Bernabé Manager V27.2.10 · Inscripción pública por temporada activa */
...
async function loadActiveSeason() {
  try {
    ...
    publicActiveSeasonId = data?.id || null;
...
async function loadTeams() {
  ...
  let query = sb.from("teams").select("name").is("deleted_at", null).order("name");
  if (publicActiveSeasonId) query = query.eq("season_id", publicActiveSeasonId);
