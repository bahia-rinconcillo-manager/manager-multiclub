...
async function extractPdfDocumentData(file){
  ...
  const data=await file.arrayBuffer(),pdf=await window.pdfjsLib.getDocument({data}).promise;
...
async function handlePdfFile(){
  ...
  let competition=competitionInput?.value.trim()||"";if(!file)return;
  if(!team){toast("Selecciona el equipo antes de importar");document.getElementById("matchesPdfFile").value="";return}
  try{
    status.textContent="Leyendo el PDF y reconociendo el calendario...";
...
function confirmPdfImport(){
  ...
  addImportedRivals(selected);saveClubMatches();document.getElementById("matchesPdfDialog")?.close();pdfParsedMatches=[];renderClubMatches();renderDashboardMatches();if(currentTeamDetailId)renderTeamDetailTab("calendar");toast(`${selected.length} partido(s) importado(s) automáticamente`);
}
