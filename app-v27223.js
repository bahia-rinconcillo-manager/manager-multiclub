// CD San Bernabé Manager · V27.2.23 · multiequipo real según cuerpo técnico + selector corregido
...
async function loadTeamUserProfiles(){
  ...
  let mappingResult=await sb.rpc("admin_list_technical_user_teams_v27223");
  if(mappingResult.error){
    // Compatibilidad: si todavía no se ha ejecutado V27.2.23, intentar el RPC anterior.
  ...
  if(!mappingResult.error&&Array.isArray(mappingResult.data)){
    ...
    if(usersSetupWarning){
      ...
      usersSetupWarning.textContent="Para detectar automáticamente todos los equipos de cada entrenador ejecuta EJECUTAR_ACTUALIZACION_V27_2_23.sql.";
    }
  }
  renderTeamUsers();
}
...
async function technicalAvailableTeams(){
  ...
  // V27.2.23: el servidor combina las asignaciones del acceso con los equipos
  // en los que esa misma persona figura en Cuerpo técnico. Así, si un entrenador
  // está registrado en dos equipos, ambos aparecen aunque el acceso antiguo solo
  // tuviera uno como equipo principal.
  let result=await sb.rpc("technical_available_teams_v27223",{p_token:token});
  if(result.error){
...
async function setTechnicalActiveTeam(teamId){
  if(!teamId)return;
  const token=getTechnicalSessionToken();
  let result=await sb.rpc("technical_select_team_v27223",{p_token:token,p_team_id:teamId});
  if(result.error){
