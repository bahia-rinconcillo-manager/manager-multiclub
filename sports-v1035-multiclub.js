    ...
    return `<div class="sports-roster-toolbar"><button type="button" data-mark-match="Convocado">Todos convocados</button><button type="button" data-mark-match="No convocado">Limpiar convocatoria</button></div>
      <div class="sports-match-roster">${list.map(player=>{
        ...
        return `<article class="sports-match-player" data-player-id="${safe(player.id)}">
          <div class="sports-player sports-player-with-photo"><img class="sports-player-photo" src="${safe(photo)}" data-photo-path="${safe(player.photo_path||"")}" alt="Foto de ${safe(player.name||"Jugador")}" onerror="this.onerror=null;this.src='assets/avatar-jugador.svg'"><div><strong>${safe(player.name||"")} ${safe(player.surname||"")}</strong><small>${safe(player.team||"")}</small></div></div>
