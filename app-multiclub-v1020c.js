// MANAGER MULTICLUB · BUILD V1.0.20 · Patrocinio completo con pagos parciales y contabilidad
...
function updateSponsorshipStatusPreview(){
  ...
  const paid=Number(sponsorshipForm.elements.paid_amount?.value||0);
...
window.openSponsorship=id=>{
  ...
  sponsorshipForm.elements.paid_amount.value=item?.paid_amount??0;
  sponsorshipForm.elements.paid_at.value=item?.paid_at||"";
  sponsorshipForm.elements.payment_method.value=item?.payment_method||"";
  sponsorshipForm.elements.document_number.value=item?.document_number||"";
...
sponsorshipForm?.elements.paid_amount?.addEventListener("input",updateSponsorshipStatusPreview);
...
sponsorshipForm?.addEventListener("submit",async e=>{
  ...
  const paidAmount=Number(fd.get("paid_amount")||0);
  if(!Number.isFinite(amount)||amount<0||!Number.isFinite(paidAmount)||paidAmount<0){
    if(submit){submit.disabled=false;submit.textContent=original}
    return toast("Los importes deben ser válidos");
  }
  if(paidAmount>amount){
    if(submit){submit.disabled=false;submit.textContent=original}
    return toast("El importe pagado no puede superar el importe total");
  }
  const payload={
    ...
    status:sponsorshipStatusFromAmounts(amount,paidAmount),
    paid_at:paidAmount>0?(String(fd.get("paid_at")||"")||null):null,
    payment_method:paidAmount>0?(String(fd.get("payment_method")||"").trim()||"Otro"):null,
  ...
  try{
    ...
    toast(paidAmount>0?`Patrocinio guardado · ${euro(paidAmount)} sincronizados con Ingresos`:"Patrocinio guardado");
    await loadAll();
...
window.deleteSponsorship=async id=>{
  const item=(sponsorships||[]).find(x=>String(x.id)===String(id));
  if(!item)return;
  const hasIncome=Number(item.paid_amount||0)>0;
...
function renderFinance(){
 ...
 financeBody.innerHTML=list.map(x=>`<tr><td>${new Date(x.movement_date+"T12:00:00").toLocaleDateString("es-ES")}</td><td>${badge(x.movement_type==="Ingreso"?"Aprobada":"Pendiente").replace(x.movement_type==="Ingreso"?"Aprobada":"Pendiente",x.movement_type)}</td><td>${x.category}</td><td>${x.concept}</td><td><strong>${euro(x.amount)}</strong></td><td>${x.payment_method||"-"}</td><td>${(x.source_payment_id||x.source_sponsorship_id||x.source_pitch_usage_id)?'<span class="auto-income-badge">Automático</span>':`<button class="mini" onclick="editMovement('${x.id}')">Editar</button>`}</td></tr>`).join("")||'<tr><td colspan="7">Sin movimientos.</td></tr>'
}
...
if(deleteMovementButton)deleteMovementButton.onclick=async()=>{
  ...
  if(x.source_payment_id||x.source_sponsorship_id||x.source_pitch_usage_id)return toast("Los movimientos automáticos no se eliminan desde Contabilidad");
